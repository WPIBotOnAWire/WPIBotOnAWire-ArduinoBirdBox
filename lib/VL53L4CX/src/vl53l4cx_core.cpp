...
void  VL53L4CX::VL53L4CX_init_version(
  VL53L4CX_DEV        Dev)
{
  ...
  pdev->version.ll_major    = VL53L4CX_LL_API_IMPLEMENTATION_VER_MAJOR;
  pdev->version.ll_minor    = VL53L4CX_LL_API_IMPLEMENTATION_VER_MINOR;
  pdev->version.ll_build    = VL53L4CX_LL_API_IMPLEMENTATION_VER_SUB;
  pdev->version.ll_revision = VL53L4CX_LL_API_IMPLEMENTATION_VER_REVISION;
}


void  VL53L4CX::VL53L4CX_init_ll_driver_state(
  VL53L4CX_DEV         Dev,
  VL53L4CX_DeviceState device_state)
{
  ...
  pstate->cfg_gph_id        = VL53L4CX_GROUPEDPARAMETERHOLD_ID_MASK;
  pstate->cfg_timing_status = 0;
  pstate->cfg_zone_id       = 0;

  pstate->rd_device_state   = device_state;
  pstate->rd_stream_count   = 0;
  pstate->rd_gph_id         = VL53L4CX_GROUPEDPARAMETERHOLD_ID_MASK;
...
VL53L4CX_Error  VL53L4CX::VL53L4CX_update_ll_driver_rd_state(
  VL53L4CX_DEV         Dev)
{
  ...
  if ((pdev->sys_ctrl.system__mode_start &
       VL53L4CX_DEVICEMEASUREMENTMODE_MODE_MASK) == 0x00) {

    pstate->rd_device_state  = VL53L4CX_DEVICESTATE_SW_STANDBY;
    pstate->rd_stream_count  = 0;
    ...
    status = VL53L4CX_update_internal_stream_counters(Dev,
                                                      pstate->rd_stream_count,
    ...
    switch (pstate->rd_device_state) {
      ...
      case VL53L4CX_DEVICESTATE_SW_STANDBY:

        if ((pdev->dyn_cfg.system__grouped_parameter_hold &
             VL53L4CX_GROUPEDPARAMETERHOLD_ID_MASK) > 0) {
          ...
          if (pstate->rd_zone_id >=
            ...
            pstate->rd_device_state =
              VL53L4CX_DEVICESTATE_RANGING_OUTPUT_DATA;
          else
            pstate->rd_device_state =
              VL53L4CX_DEVICESTATE_RANGING_GATHER_DATA;
        }
      ...
      case VL53L4CX_DEVICESTATE_RANGING_WAIT_GPH_SYNC:
        pstate->rd_stream_count = 0;
        pstate->rd_internal_stream_count = 0;
        pstate->rd_internal_stream_count_val = 0;
        pstate->rd_zone_id      = 0;
      ...
      case VL53L4CX_DEVICESTATE_RANGING_GATHER_DATA:
        pstate->rd_zone_id++;
        if (pstate->rd_zone_id >=
            pdev->zone_cfg.active_zones)
          pstate->rd_device_state =
      ...
      case VL53L4CX_DEVICESTATE_RANGING_OUTPUT_DATA:
        pstate->rd_zone_id        = 0;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_check_ll_driver_rd_state(
  VL53L4CX_DEV         Dev)
{
  ...
  VL53L4CX_ll_driver_state_t  *pstate       = &(pdev->ll_state);
  VL53L4CX_system_results_t   *psys_results = &(pdev->sys_results);
  VL53L4CX_histogram_bin_data_t *phist_data = &(pdev->hist_data);
  VL53L4CX_zone_private_dyn_cfgs_t *pZ = &(pres->zone_dyn_cfgs);
  ...
  device_gph_id = (psys_results->result__interrupt_status &
                   VL53L4CX_INTERRUPT_STATUS__GPH_ID_INT_STATUS_MASK) >> 4;
  ...
  if (pstate->rd_device_state ==
    ...
    if (histogram_mode == 0) {
      if (device_range_status !=
          VL53L4CX_DEVICEERROR_GPHSTREAMCOUNT0READY)
        status =
          VL53L4CX_ERROR_GPH_SYNC_CHECK_FAIL;

    }
  } else {
    if (pstate->rd_stream_count != device_stream_count) {
      status = VL53L4CX_ERROR_STREAM_COUNT_CHECK_FAIL;
    }


    if (pstate->rd_gph_id != device_gph_id) {
      status = VL53L4CX_ERROR_GPH_ID_CHECK_FAIL;
    }
    ...
    expected_stream_count =
      pZ->VL53L4CX_p_003[pstate->rd_zone_id].expected_stream_count;
    expected_gph_id =
      pZ->VL53L4CX_p_003[pstate->rd_zone_id].expected_gph_id;
    ...
    if (expected_stream_count != device_stream_count) {
      ...
      if (!((pdev->zone_cfg.active_zones == 0) &&
        ...
        status =
          VL53L4CX_ERROR_ZONE_STREAM_COUNT_CHECK_FAIL;
    ...
    if (expected_gph_id != device_gph_id) {
      status = VL53L4CX_ERROR_ZONE_GPH_ID_CHECK_FAIL;
    }
...
VL53L4CX_Error  VL53L4CX::VL53L4CX_update_ll_driver_cfg_state(
  VL53L4CX_DEV         Dev)
{
  ...
  if ((pdev->sys_ctrl.system__mode_start &
    ...
    pstate->cfg_device_state  = VL53L4CX_DEVICESTATE_SW_STANDBY;
    pstate->cfg_stream_count  = 0;
    ...
    status = VL53L4CX_update_internal_stream_counters(
               Dev,
               pstate->cfg_stream_count,
    ...
    switch (pstate->cfg_device_state) {
      ...
      case VL53L4CX_DEVICESTATE_SW_STANDBY:
        ...
        pstate->cfg_device_state =
          VL53L4CX_DEVICESTATE_RANGING_DSS_AUTO;
        break;

      case VL53L4CX_DEVICESTATE_RANGING_DSS_AUTO:
        pstate->cfg_zone_id++;
        if (pstate->cfg_zone_id >
            pdev->zone_cfg.active_zones) {
      ...
      case VL53L4CX_DEVICESTATE_RANGING_DSS_MANUAL:
        pstate->cfg_zone_id++;
        if (pstate->cfg_zone_id >
            pdev->zone_cfg.active_zones) {
          pstate->cfg_zone_id = 0;
      ...
      default:
        ...
        pstate->cfg_internal_stream_count_val = 0;
        pstate->cfg_gph_id =
          VL53L4CX_GROUPEDPARAMETERHOLD_ID_MASK;
        pstate->cfg_timing_status = 0;
        pstate->cfg_zone_id       = 0;
        break;
    }
  ...
  if (pdev->zone_cfg.active_zones == 0) {
    ...
    pZ->VL53L4CX_p_003[prev_cfg_zone_id].expected_stream_count
      = prev_cfg_stream_count - 1;

    pZ->VL53L4CX_p_003[pstate->rd_zone_id].expected_gph_id =
      prev_cfg_gph_id ^ VL53L4CX_GROUPEDPARAMETERHOLD_ID_MASK;
  } else {
    pZ->VL53L4CX_p_003[prev_cfg_zone_id].expected_stream_count
      = prev_cfg_stream_count;
    pZ->VL53L4CX_p_003[prev_cfg_zone_id].expected_gph_id =
      prev_cfg_gph_id;
  }
...
void VL53L4CX::VL53L4CX_copy_rtn_good_spads_to_buffer(
...
void VL53L4CX::VL53L4CX_init_system_results(
...
void VL53L4CX::V53L1_init_zone_results_structure(
  ...
  VL53L4CX_zone_results_t  *pdata)
{
  ...
  pdata->max_zones    = VL53L4CX_MAX_USER_ZONES;
  pdata->active_zones = active_zones;

  for (z = 0; z < pdata->max_zones; z++) {
    pobjects = &(pdata->VL53L4CX_p_003[z]);
    pobjects->cfg_device_state = VL53L4CX_DEVICESTATE_SW_STANDBY;
    pobjects->rd_device_state  = VL53L4CX_DEVICESTATE_SW_STANDBY;
    pobjects->max_objects      = VL53L4CX_MAX_RANGE_RESULTS;
    pobjects->active_objects   = 0;
  }
}

void VL53L4CX::V53L1_init_zone_dss_configs(
  VL53L4CX_DEV              Dev)
{
  ...
  VL53L4CX_LLDriverResults_t  *pres =
    VL53L4CXDevStructGetLLResultsHandle(Dev);
  uint8_t  z = 0;
  uint8_t max_zones    = VL53L4CX_MAX_USER_ZONES;
  VL53L4CX_zone_private_dyn_cfgs_t *pdata = &(pres->zone_dyn_cfgs);

  for (z = 0; z < max_zones; z++) {
    pdata->VL53L4CX_p_003[z].dss_mode =
      VL53L4CX_DSS_CONTROL__MODE_TARGET_RATE;
    pdata->VL53L4CX_p_003[z].dss_requested_effective_spad_count = 0;
  }
}


void VL53L4CX::VL53L4CX_init_histogram_config_structure(
  uint8_t   even_bin0,
  uint8_t   even_bin1,
  uint8_t   even_bin2,
  uint8_t   even_bin3,
...
void VL53L4CX::VL53L4CX_init_histogram_multizone_config_structure(
  uint8_t   even_bin0,
  uint8_t   even_bin1,
  uint8_t   even_bin2,
  uint8_t   even_bin3,
...
void VL53L4CX::VL53L4CX_init_xtalk_bin_data_struct(
  uint32_t                        bin_value,
  ...
  VL53L4CX_xtalk_histogram_shape_t *pdata)
{
  ...
  pdata->VL53L4CX_p_020               = VL53L4CX_XTALK_HISTO_BINS;
  pdata->VL53L4CX_p_021            = (uint8_t)VL53L4CX_p_021;
...
void VL53L4CX::VL53L4CX_i2c_encode_uint16_t(
  uint16_t    ip_value,
  uint16_t    count,
  uint8_t    *pbuffer)
{
  ...
  uint16_t   i    = 0;
  uint16_t   VL53L4CX_p_003 = 0;

  VL53L4CX_p_003 =  ip_value;

  for (i = 0; i < count; i++) {
    pbuffer[count - i - 1] = (uint8_t)(VL53L4CX_p_003 & 0x00FF);
    VL53L4CX_p_003 = VL53L4CX_p_003 >> 8;
  }
}

uint16_t VL53L4CX::VL53L4CX_i2c_decode_uint16_t(
  uint16_t    count,
  uint8_t    *pbuffer)
{
...
void VL53L4CX::VL53L4CX_i2c_encode_int16_t(
  int16_t     ip_value,
  uint16_t    count,
  uint8_t    *pbuffer)
{
...
int16_t VL53L4CX::VL53L4CX_i2c_decode_int16_t(
  uint16_t    count,
  uint8_t    *pbuffer)
{
...
void VL53L4CX::VL53L4CX_i2c_encode_uint32_t(
  uint32_t    ip_value,
  uint16_t    count,
  uint8_t    *pbuffer)
{
...
uint32_t VL53L4CX::VL53L4CX_i2c_decode_uint32_t(
  uint16_t    count,
  uint8_t    *pbuffer)
{
...
uint32_t VL53L4CX::VL53L4CX_i2c_decode_with_mask(
  uint16_t    count,
  uint8_t    *pbuffer,
  uint32_t    bit_mask,
  uint32_t    down_shift,
...
void VL53L4CX::VL53L4CX_i2c_encode_int32_t(
  int32_t     ip_value,
  uint16_t    count,
  uint8_t    *pbuffer)
{
...
int32_t VL53L4CX::VL53L4CX_i2c_decode_int32_t(
  uint16_t    count,
  uint8_t    *pbuffer)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_start_test(
  VL53L4CX_DEV    Dev,
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_firmware_enable_register(
  VL53L4CX_DEV    Dev,
  uint8_t       value)
{
  ...
  status = VL53L4CX_WrByte(
             Dev,
             VL53L4CX_FIRMWARE__ENABLE,
             pdev->sys_ctrl.firmware__enable);

  return status;
}

VL53L4CX_Error VL53L4CX::VL53L4CX_enable_firmware(
  VL53L4CX_DEV    Dev)
{
  ...
  status = VL53L4CX_set_firmware_enable_register(Dev, 0x01);

  return status;
}


VL53L4CX_Error VL53L4CX::VL53L4CX_disable_firmware(
  VL53L4CX_DEV    Dev)
{
  ...
  status = VL53L4CX_set_firmware_enable_register(Dev, 0x00);

  return status;
}


VL53L4CX_Error VL53L4CX::VL53L4CX_set_powerforce_register(
  VL53L4CX_DEV    Dev,
  uint8_t       value)
{
  ...
  status = VL53L4CX_WrByte(
             Dev,
             VL53L4CX_POWER_MANAGEMENT__GO1_POWER_FORCE,
...
VL53L4CX_Error VL53L4CX::VL53L4CX_enable_powerforce(
  VL53L4CX_DEV    Dev)
{
  ...
  status = VL53L4CX_set_powerforce_register(Dev, 0x01);

  return status;
}


VL53L4CX_Error VL53L4CX::VL53L4CX_disable_powerforce(
  VL53L4CX_DEV    Dev)
{
  ...
  status = VL53L4CX_set_powerforce_register(Dev, 0x00);

  return status;
}


VL53L4CX_Error VL53L4CX::VL53L4CX_clear_interrupt(
  VL53L4CX_DEV    Dev)
{
  ...
  pdev->sys_ctrl.system__interrupt_clear = VL53L4CX_CLEAR_RANGE_INT;

  status = VL53L4CX_WrByte(
             Dev,
             VL53L4CX_SYSTEM__INTERRUPT_CLEAR,
  ...
  VL53L4CX_DEV    Dev)
{
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    status = VL53L4CX_WrByte(
               Dev,
               VL53L4CX_SHADOW_RESULT__STREAM_COUNT,
               0x00);
  }
  ...
  uint8_t   VL53L4CX_p_005)
{
  ...
  uint8_t   VL53L4CX_p_030   = 0;
  uint32_t  macro_period_us      = 0;

  pll_period_us = VL53L4CX_calc_pll_period_us(fast_osc_frequency);



  VL53L4CX_p_030 = VL53L4CX_decode_vcsel_period(VL53L4CX_p_005);



  macro_period_us =
    (uint32_t)VL53L4CX_MACRO_PERIOD_VCSEL_PERIODS *
    pll_period_us;
  macro_period_us = macro_period_us >> 6;

  macro_period_us = macro_period_us * (uint32_t)VL53L4CX_p_030;
  macro_period_us = macro_period_us >> 6;
...
uint16_t VL53L4CX::VL53L4CX_calc_range_ignore_threshold(
...
uint32_t VL53L4CX::VL53L4CX_calc_timeout_mclks(
  uint32_t timeout_us,
...
uint16_t VL53L4CX::VL53L4CX_calc_encoded_timeout(
  uint32_t timeout_us,
  uint32_t macro_period_us)
{
  ...
  timeout_mclks   =
    VL53L4CX_calc_timeout_mclks(timeout_us, macro_period_us);

  timeout_encoded =
    VL53L4CX_encode_timeout(timeout_mclks);

  return timeout_encoded;
}


uint32_t VL53L4CX::VL53L4CX_calc_timeout_us(
...
uint32_t VL53L4CX::VL53L4CX_calc_crosstalk_plane_offset_with_margin(
...
uint32_t VL53L4CX::VL53L4CX_calc_decoded_timeout_us(
  ...
  uint32_t macro_period_us)
{
  ...
  timeout_mclks =
    VL53L4CX_decode_timeout(timeout_encoded);

  timeout_us    =
    VL53L4CX_calc_timeout_us(timeout_mclks, macro_period_us);

  return timeout_us;
}


uint16_t VL53L4CX::VL53L4CX_encode_timeout(uint32_t timeout_mclks)
{
...
uint32_t VL53L4CX::VL53L4CX_decode_timeout(uint16_t encoded_timeout)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_calc_timeout_register_values(
  ...
  VL53L4CX_timing_config_t  *ptiming)
{
  ...
  if (fast_osc_frequency == 0) {
    ...
    timeout_encoded = VL53L4CX_calc_encoded_timeout(
                        range_config_timeout_us,
                        macro_period_us);
...
uint8_t VL53L4CX::VL53L4CX_encode_vcsel_period(uint8_t VL53L4CX_p_030)
{
  ...
  vcsel_period_reg = (VL53L4CX_p_030 >> 1) - 1;
  ...
  uint8_t  *pbuffer)
{
  ...
  VL53L4CX_p_003 = ip_value;
  for (i = 0; i < no_of_bytes; i++) {
    pbuffer[no_of_bytes - i - 1] = VL53L4CX_p_003 & 0x00FF;
    VL53L4CX_p_003 = VL53L4CX_p_003 >> 8;
  }
}
*/
...
void  VL53L4CX::VL53L4CX_hist_get_bin_sequence_config(
  VL53L4CX_DEV                     Dev,
  VL53L4CX_histogram_bin_data_t   *pdata)
{
  ...
  for (i = 0; i < VL53L4CX_MAX_BIN_SEQUENCE_LENGTH; i++) {
    pdata->bin_rep[i] = 1;
  }
...
VL53L4CX_Error  VL53L4CX::VL53L4CX_hist_phase_consistency_check(
  VL53L4CX_DEV                   Dev,
  ...
  VL53L4CX_range_results_t      *prange_curr)
{
  ...
  VL53L4CX_DeviceError  range_status = 0;
  ...
  for (lc = 0; lc < prange_curr->active_results; lc++) {
    ...
    if (!((prange_curr->VL53L4CX_p_003[lc].range_status ==
           VL53L4CX_DEVICEERROR_RANGECOMPLETE) ||
          (prange_curr->VL53L4CX_p_003[lc].range_status ==
    ...
    if (prange_prev->active_objects == 0)
      prange_curr->VL53L4CX_p_003[lc].range_status =
        VL53L4CX_DEVICEERROR_PREV_RANGE_NO_TARGETS;
    else
      prange_curr->VL53L4CX_p_003[lc].range_status =
        VL53L4CX_DEVICEERROR_PHASECONSISTENCY;
    ...
    for (p = 0; p < prange_prev->active_objects; p++) {
      ...
      if (prange_curr->VL53L4CX_p_003[lc].VL53L4CX_p_011 >
          prange_prev->VL53L4CX_p_003[p].VL53L4CX_p_011) {
        phase_delta =
          prange_curr->VL53L4CX_p_003[lc].VL53L4CX_p_011 -
          prange_prev->VL53L4CX_p_003[p].VL53L4CX_p_011;
      } else {
        phase_delta =
          prange_prev->VL53L4CX_p_003[p].VL53L4CX_p_011 -
      ...
      if (phase_delta < phase_tolerance) {
        ...
        if (status == VL53L4CX_ERROR_NONE)
          status =
            VL53L4CX_hist_events_consistency_check(
              event_sigma,
              event_min_spad_count,
              phist_prev,
              &(prange_prev->VL53L4CX_p_003[p]),
              &(prange_curr->VL53L4CX_p_003[lc]),
              &events_tolerance,
              &events_delta,
              &range_status);
        ...
        if (status == VL53L4CX_ERROR_NONE &&
            ...
            VL53L4CX_DEVICEERROR_RANGECOMPLETE)
          status =
            VL53L4CX_hist_merged_pulse_check(
              min_max_tolerance,
              &(prange_curr->VL53L4CX_p_003[lc]),
              &range_status);
...
VL53L4CX_Error  VL53L4CX::VL53L4CX_hist_events_consistency_check(
  uint8_t                      event_sigma,
  ...
  VL53L4CX_DeviceError          *prange_status)
{
  ...
  if (event_sigma == 0) {
    *prange_status = VL53L4CX_DEVICEERROR_RANGECOMPLETE;
    return status;
  }
  ...
  tmpc *= (int64_t)prange_curr->VL53L4CX_p_004;
  ...
  c_signal_events  = (int64_t)prange_curr->VL53L4CX_p_017;
  c_signal_events -= (int64_t)prange_curr->VL53L4CX_p_016;
  c_signal_events *= (int64_t)events_scaler;
  c_signal_events += 2048;
  c_signal_events /= 4096;

  c_sig_noise_sq  = (int64_t)events_scaler_sq;
  c_sig_noise_sq *= (int64_t)prange_curr->VL53L4CX_p_017;
  c_sig_noise_sq += 2048;
  c_sig_noise_sq /= 4096;

  c_amb_noise_sq  = (int64_t)events_scaler_sq;
  c_amb_noise_sq *= (int64_t)prange_curr->VL53L4CX_p_016;
  c_amb_noise_sq += 2048;
  c_amb_noise_sq /= 4096;
  ...
  p_amb_noise_sq  =
    (int64_t)prange_prev->VL53L4CX_p_016;
  ...
  noise_sq_sum =
    (uint32_t)prange_prev->VL53L4CX_p_017 +
    (uint32_t)c_sig_noise_sq +
    (uint32_t)p_amb_noise_sq +
    (uint32_t)c_amb_noise_sq;

  *pevents_tolerance =
    (int32_t)VL53L4CX_isqrt(noise_sq_sum * 16);

  *pevents_tolerance *= (int32_t)event_sigma;
  *pevents_tolerance += 32;
  *pevents_tolerance /= 64;

  p_signal_events  = (int32_t)prange_prev->VL53L4CX_p_017;
  p_signal_events -= (int32_t)prange_prev->VL53L4CX_p_016;
  ...
  if (*pevents_delta > *pevents_tolerance &&
      prange_curr->VL53L4CX_p_004 > min_effective_spad_count) {
    *prange_status = VL53L4CX_DEVICEERROR_EVENTCONSISTENCY;
  } else {
    *prange_status = VL53L4CX_DEVICEERROR_RANGECOMPLETE;
  }
...
VL53L4CX_Error  VL53L4CX::VL53L4CX_hist_merged_pulse_check(
  ...
  VL53L4CX_DeviceError          *prange_status)
{
  ...
  if (min_max_tolerance_mm > 0 &&
    ...
    *prange_status = VL53L4CX_DEVICEERROR_RANGECOMPLETE_MERGED_PULSE;
  } else {
    *prange_status = VL53L4CX_DEVICEERROR_RANGECOMPLETE;
  }
...
VL53L4CX_Error  VL53L4CX::VL53L4CX_hist_xmonitor_consistency_check(
  VL53L4CX_DEV                   Dev,
  VL53L4CX_zone_hist_info_t     *phist_prev,
  VL53L4CX_zone_objects_t       *prange_prev,
  VL53L4CX_range_data_t         *prange_curr)
{
  ...
  if (prange_curr->range_status == VL53L4CX_DEVICEERROR_RANGECOMPLETE ||
      prange_curr->range_status ==
      VL53L4CX_DEVICEERROR_RANGECOMPLETE_NO_WRAP_CHECK ||
      prange_curr->range_status ==
      VL53L4CX_DEVICEERROR_EVENTCONSISTENCY) {
...
VL53L4CX_Error  VL53L4CX::VL53L4CX_hist_wrap_dmax(
  ...
  int16_t                            *pwrap_dmax_mm)
{
  ...
  *pwrap_dmax_mm = 0;
  ...
  if (pcurrent->VL53L4CX_p_015 != 0) {



    pll_period_mm =
      VL53L4CX_calc_pll_period_mm(
        pcurrent->VL53L4CX_p_015);
...
void VL53L4CX::VL53L4CX_hist_combine_mm1_mm2_offsets(
...
VL53L4CX_Error VL53L4CX::VL53L4CX_hist_xtalk_extract_calc_window(
  ...
  VL53L4CX_hist_xtalk_extract_data_t   *pxtalk_data)
{
  ...
  pxtalk_data->pll_period_mm =
    VL53L4CX_calc_pll_period_mm(phist_bins->VL53L4CX_p_015);
  ...
  pxtalk_data->VL53L4CX_p_012 =
    (uint8_t)(pxtalk_data->xtalk_start_phase / 2048);


  pxtalk_data->VL53L4CX_p_013 =
  ...
  if (pxtalk_data->VL53L4CX_p_013 > (pxtalk_data->target_start - 1)) {
    pxtalk_data->VL53L4CX_p_013 = pxtalk_data->target_start - 1;
  }


  pxtalk_data->effective_width =
    (2048 * ((int32_t)pxtalk_data->VL53L4CX_p_013 + 1));
...
VL53L4CX_Error VL53L4CX::VL53L4CX_hist_xtalk_extract_calc_event_sums(
  VL53L4CX_histogram_bin_data_t        *phist_bins,
  VL53L4CX_hist_xtalk_extract_data_t   *pxtalk_data)
{
  ...
  for (lb  = pxtalk_data->VL53L4CX_p_012;
       lb <= pxtalk_data->VL53L4CX_p_013;
       lb++) {


    i = (lb + phist_bins->number_of_ambient_bins +
         phist_bins->VL53L4CX_p_021) %
        phist_bins->VL53L4CX_p_021;


    pxtalk_data->signal_events_sum += phist_bins->bin_data[i];
    pxtalk_data->signal_events_sum -=
      phist_bins->VL53L4CX_p_028;
  }



  for (lb  = 0; lb < VL53L4CX_XTALK_HISTO_BINS  &&
       lb < phist_bins->VL53L4CX_p_021; lb++) {
...
VL53L4CX_Error VL53L4CX::VL53L4CX_hist_xtalk_extract_calc_rate_per_spad(
  VL53L4CX_hist_xtalk_extract_data_t   *pxtalk_data)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_hist_xtalk_extract_calc_shape(
  VL53L4CX_hist_xtalk_extract_data_t  *pxtalk_data,
  VL53L4CX_xtalk_histogram_shape_t    *pxtalk_shape)
{
  ...
  pxtalk_shape->VL53L4CX_p_019      = 0;
  pxtalk_shape->VL53L4CX_p_020    = VL53L4CX_XTALK_HISTO_BINS;
  pxtalk_shape->VL53L4CX_p_021 = VL53L4CX_XTALK_HISTO_BINS;
  ...
  for (lb = 0; lb < VL53L4CX_XTALK_HISTO_BINS; lb++) {

    if ((lb < (int32_t)pxtalk_data->VL53L4CX_p_012 ||
         lb > (int32_t)pxtalk_data->VL53L4CX_p_013)  ||
  ...
  VL53L4CX_xtalk_histogram_shape_t  *pxtalk_shape)
{
  ...
  for (lb = 0; lb < VL53L4CX_XTALK_HISTO_BINS; lb++) {
    ...
    if (lb == bin_start && lb == bin_stop) {
      VL53L4CX_p_018 =
        VL53L4CX_hist_xtalk_shape_model_interp(
          events_per_bin,
    ...
    pxtalk_shape->bin_data[lb] = VL53L4CX_p_018;
  }
  ...
  uint32_t      phase_delta)
{
  ...
  uint32_t  VL53L4CX_p_018  = 0;

  VL53L4CX_p_018  = (uint32_t)events_per_bin * phase_delta;
  VL53L4CX_p_018 +=  1024;
  VL53L4CX_p_018 /=  2048;


  if (VL53L4CX_p_018 > 0xFFFFU) {
    VL53L4CX_p_018 = 0xFFFFU;
  }

  return (uint16_t)VL53L4CX_p_018;
}


void VL53L4CX::VL53L4CX_spad_number_to_byte_bit_index(
  uint8_t  spad_number,
  uint8_t *pbyte_index,
  uint8_t *pbit_index,
  uint8_t *pbit_mask)
...
void VL53L4CX::VL53L4CX_encode_row_col(
  uint8_t  row,
  uint8_t  col,
...
void VL53L4CX::VL53L4CX_decode_zone_size(
...
void VL53L4CX::VL53L4CX_encode_zone_size(
  uint8_t  width,
  uint8_t  height,
...
void VL53L4CX::VL53L4CX_decode_zone_limits(
  ...
  int16_t  *py_ur)
{
  ...
  *px_ur = *px_ll + (int16_t)width;
  if (*px_ur > (VL53L4CX_SPAD_ARRAY_WIDTH - 1)) {
    *px_ur = VL53L4CX_SPAD_ARRAY_WIDTH - 1;
  }
  ...
  *py_ur = *py_ll + (int16_t)height;
  if (*py_ur > (VL53L4CX_SPAD_ARRAY_HEIGHT - 1)) {
    *py_ur = VL53L4CX_SPAD_ARRAY_HEIGHT - 1;
  }
}


uint8_t VL53L4CX::VL53L4CX_is_aperture_location(
  uint8_t row,
  uint8_t col)
{
...
void VL53L4CX::VL53L4CX_calc_max_effective_spads(
  uint8_t     encoded_zone_centre,
  uint8_t     encoded_zone_size,
  uint8_t    *pgood_spads,
  uint16_t    aperture_attenuation,
  ...
  *pmax_effective_spads = 0;
  ...
  for (y = zone_y_ll; y <= zone_y_ur; y++) {
    for (x = zone_x_ll; x <= zone_x_ur; x++) {
      ...
      VL53L4CX_spad_number_to_byte_bit_index(
        spad_number,
        &byte_index,
        &bit_index,
        &bit_mask);
      ...
      if ((pgood_spads[byte_index] & bit_mask) > 0) {
        ...
        is_aperture = VL53L4CX_is_aperture_location(
                        (uint8_t)y,
                        (uint8_t)x);
...
void VL53L4CX::VL53L4CX_calc_mm_effective_spads(
  uint8_t     encoded_mm_roi_centre,
  uint8_t     encoded_mm_roi_size,
  uint8_t     encoded_zone_centre,
  uint8_t     encoded_zone_size,
...
void VL53L4CX::VL53L4CX_hist_copy_results_to_sys_and_core(
  ...
  VL53L4CX_core_results_t            *pcore)
{
  ...
  VL53L4CX_init_system_results(psys);
  ...
  for (i = 0; i < phist->active_results; i++) {
    ...
    switch (i) {
      case 0:
        ...
        psys->result__sigma_sd0 = pdata->VL53L4CX_p_002;
        psys->result__phase_sd0 = pdata->VL53L4CX_p_011;
      ...
      case 1:
        ...
        psys->result__sigma_sd1 = pdata->VL53L4CX_p_002;
        psys->result__phase_sd1 = pdata->VL53L4CX_p_011;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_sum_histogram_data(
  ...
  VL53L4CX_histogram_bin_data_t *phist_output)
{
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    if (phist_output->VL53L4CX_p_021 >=
        phist_input->VL53L4CX_p_021) {
      smallest_bin_num = phist_input->VL53L4CX_p_021;
    } else {
      smallest_bin_num = phist_output->VL53L4CX_p_021;
    }
  }
  ...
  if (status == VL53L4CX_ERROR_NONE)
    phist_output->VL53L4CX_p_028 +=
      phist_input->VL53L4CX_p_028;

  return status;
}


VL53L4CX_Error VL53L4CX::VL53L4CX_avg_histogram_data(
  ...
  VL53L4CX_histogram_bin_data_t *phist_avg)
{
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    for (i = 0; i < phist_sum->VL53L4CX_p_021; i++) {
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    if (no_of_samples > 0)
      phist_avg->VL53L4CX_p_028 =
        phist_sum->VL53L4CX_p_028 /
        (int32_t)no_of_samples;
    else
      phist_avg->VL53L4CX_p_028 =
        phist_sum->VL53L4CX_p_028;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_save_cfg_data(
  VL53L4CX_DEV  Dev)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_zone_update(
  VL53L4CX_DEV  Dev,
  VL53L4CX_range_results_t *presults)
{
  ...
  pZ->VL53L4CX_p_003[zone_id].dss_requested_effective_spad_count = 0;

  max_total_rate_per_spads =
    presults->VL53L4CX_p_003[0].total_rate_per_spad_mcps;

  for (i = 1; i < presults->active_results; i++) {
    if (presults->VL53L4CX_p_003[i].total_rate_per_spad_mcps >
        max_total_rate_per_spads)
      max_total_rate_per_spads =
        presults->VL53L4CX_p_003[i].total_rate_per_spad_mcps;
  ...
  pZ->VL53L4CX_p_003[zone_id].dss_requested_effective_spad_count =
    (uint16_t)temp;

  return status;
}

VL53L4CX_Error VL53L4CX::VL53L4CX_multizone_hist_bins_update(
  VL53L4CX_DEV  Dev)
{
  ...
  VL53L4CX_histogram_config_t *phist_cfg = &(pdev->hist_cfg);
    ...
    if (next_range_is_odd_timing) {
      ...
      phist_cfg->histogram_config__low_amb_odd_bin_4_5  =
        pmulti_hist->histogram_config__low_amb_even_bin_4_5;
             ...
             VL53L4CX_ZONECONFIG_BINCONFIG__MIDAMB) {
    if (!next_range_is_odd_timing) {
      phist_cfg->histogram_config__low_amb_even_bin_0_1  =
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    VL53L4CX_copy_hist_bins_to_static_cfg(
      phist_cfg,
      &(pdev->stat_cfg),
      &(pdev->tim_cfg));
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_update_internal_stream_counters(
  VL53L4CX_DEV  Dev,
...
VL53L4CX_GPIO_interrupt_config_t VL53L4CX::VL53L4CX_decode_GPIO_interrupt_config(
  uint8_t   system__interrupt_config)
{
  VL53L4CX_GPIO_interrupt_config_t  intconf;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_GPIO_rate_threshold(
  VL53L4CX_DEV                      Dev,
  uint16_t      threshold_high,
  uint16_t      threshold_low)
{
  ...
  VL53L4CX_GPIO_interrupt_config_t *pintconf)
{
  ...
  status = VL53L4CX_set_GPIO_distance_threshold(
             Dev,
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    status =
      VL53L4CX_set_GPIO_rate_threshold(
        Dev,
  ...
  uint16_t      fast_osc_frequency)
{
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_WrByte(
        Dev,
        VL53L4CX_PHASECAL_CONFIG__TIMEOUT_MACROP,
        pdev->gen_cfg.phasecal_config__timeout_macrop);
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_WrByte(
        Dev,
        VL53L4CX_RANGE_CONFIG__VCSEL_PERIOD_A,
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_WrWord(
        Dev,
        VL53L4CX_REF_SPAD_CHAR__TOTAL_RATE_TARGET_MCPS,
        total_rate_target_mcps);
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_WrWord(
        Dev,
        VL53L4CX_RANGE_CONFIG__SIGMA_THRESH,
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_WrWord(
        Dev,
        VL53L4CX_RANGE_CONFIG__MIN_COUNT_RATE_RTN_LIMIT_MCPS,
  ...
  uint16_t              fast_osc_frequency)
{
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_WrByte(
        Dev,
        VL53L4CX_CAL_CONFIG__VCSEL_START,
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_WrByte(
        Dev,
        VL53L4CX_GLOBAL_CONFIG__VCSEL_WIDTH,
  ...
  buffer[2] = pssc_cfg->VL53L4CX_p_005;
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_WriteMulti(
        Dev,
        VL53L4CX_RANGE_CONFIG__TIMEOUT_MACROP_B_HI,
        buffer,
        5);



  buffer[0] = pssc_cfg->VL53L4CX_p_005;
  buffer[1] = pssc_cfg->VL53L4CX_p_005;
  ...
  VL53L4CX_spad_rate_data_t  *pspad_rates)
{
  ...
  uint8_t  VL53L4CX_p_003[512];
  uint8_t *pdata = &VL53L4CX_p_003[0];
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_ReadMulti(
        Dev,
        VL53L4CX_PRIVATE__PATCH_BASE_ADDR_RSLV,
        pdata,
        512);


  pdata = &VL53L4CX_p_003[0];
  for (i = 0; i < VL53L4CX_NO_OF_SPAD_ENABLES; i++) {
    pspad_rates->rate_data[i] =
      (uint16_t)VL53L4CX_decode_unsigned_integer(pdata, 2);
    pdata += 2;
  }



  pspad_rates->VL53L4CX_p_020     = VL53L4CX_NO_OF_SPAD_ENABLES;
  pspad_rates->no_of_values    = VL53L4CX_NO_OF_SPAD_ENABLES;
...
)
{
  ...
  VL53L4CX_LLDriverData_t *pdev = VL53L4CXDevStructGetLLDriverHandle(Dev);
  VL53L4CX_LLDriverResults_t *pres = VL53L4CXDevStructGetLLResultsHandle(Dev);
  VL53L4CX_smudge_corrector_config_t *pconfig =
    &(pdev->smudge_correct_config);
  VL53L4CX_smudge_corrector_internals_t *pint =
    &(pdev->smudge_corrector_internals);

  VL53L4CX_range_results_t *presults = &(pres->range_results);
  VL53L4CX_range_data_t *pxmonitor = &(presults->xmonitor);
  ...
  temp64a = pxmonitor->VL53L4CX_p_017 +
            pxmonitor->VL53L4CX_p_016;
  ...
  temp64z = pconfig->noise_margin * pxmonitor->VL53L4CX_p_004;
  if (temp64z == 0) {
    temp64z = 1;
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_calc_new_xtalk(
  VL53L4CX_DEV        Dev,
...
)
{
  ...
  VL53L4CX_xtalk_config_t  *pX = &(pdev->xtalk_cfg);
  VL53L4CX_xtalk_calibration_results_t  *pC = &(pdev->xtalk_cal);
  uint32_t *pcpo;
...
#define CONT_RESET  2
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_corrector(
  VL53L4CX_DEV                          Dev
)
{
  ...
  VL53L4CX_range_results_t  *pR = &(pres->range_results);
  VL53L4CX_xtalk_config_t  *pX = &(pdev->xtalk_cfg);
  ...
  VL53L4CX_dynamic_xtalk_correction_output_init(pres);
  ...
  if ((pR->xmonitor.range_status
    ...
    for (i = 0; i < pR->active_results; i++) {
      if (pR->VL53L4CX_p_003[i].range_status ==
          VL53L4CX_DEVICEERROR_RANGECOMPLETE) {
        if (pR->VL53L4CX_p_003[i].median_range_mm
            <=
    ...
    amr =
      pR->VL53L4CX_p_003[nodetect_index].ambient_count_rate_mcps;
  ...
  if (run_smudge_detection) {
    ...
    VL53L4CX_dynamic_xtalk_correction_calc_required_samples(Dev);


    xtalk_offset_in =
      pR->xmonitor.VL53L4CX_p_009;
    ...
    if (continue_processing == CONT_CONTINUE) {
      ...
      VL53L4CX_dynamic_xtalk_correction_calc_new_xtalk(
        Dev,
        xtalk_offset_out,
        pconfig,
        pout,
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_data_init(
  VL53L4CX_DEV                          Dev
)
{
  ...
  pdev->smudge_correct_config.smudge_corr_single_apply  =
    VL53L4CX_TUNINGPARM_DYNXTALK_SMUDGE_COR_SINGLE_APPLY_DEFAULT;

  pdev->smudge_correct_config.smudge_margin =
    VL53L4CX_TUNINGPARM_DYNXTALK_SMUDGE_MARGIN_DEFAULT;
  pdev->smudge_correct_config.noise_margin =
    VL53L4CX_TUNINGPARM_DYNXTALK_NOISE_MARGIN_DEFAULT;
  pdev->smudge_correct_config.user_xtalk_offset_limit =
    VL53L4CX_TUNINGPARM_DYNXTALK_XTALK_OFFSET_LIMIT_DEFAULT;
  pdev->smudge_correct_config.user_xtalk_offset_limit_hi =
    VL53L4CX_TUNINGPARM_DYNXTALK_XTALK_OFFSET_LIMIT_HI_DEFAULT;
  pdev->smudge_correct_config.sample_limit =
    VL53L4CX_TUNINGPARM_DYNXTALK_SAMPLE_LIMIT_DEFAULT;
  pdev->smudge_correct_config.single_xtalk_delta =
    VL53L4CX_TUNINGPARM_DYNXTALK_SINGLE_XTALK_DELTA_DEFAULT;
  pdev->smudge_correct_config.averaged_xtalk_delta =
    VL53L4CX_TUNINGPARM_DYNXTALK_AVERAGED_XTALK_DELTA_DEFAULT;
  pdev->smudge_correct_config.smudge_corr_clip_limit =
    VL53L4CX_TUNINGPARM_DYNXTALK_CLIP_LIMIT_DEFAULT;
  pdev->smudge_correct_config.smudge_corr_ambient_threshold =
    VL53L4CX_TUNINGPARM_DYNXTALK_XTALK_AMB_THRESHOLD_DEFAULT;
  pdev->smudge_correct_config.scaler_calc_method =
    0;
  pdev->smudge_correct_config.x_gradient_scaler =
    VL53L4CX_TUNINGPARM_DYNXTALK_XGRADIENT_SCALER_DEFAULT;
  pdev->smudge_correct_config.y_gradient_scaler =
    VL53L4CX_TUNINGPARM_DYNXTALK_YGRADIENT_SCALER_DEFAULT;
  pdev->smudge_correct_config.user_scaler_set =
    VL53L4CX_TUNINGPARM_DYNXTALK_USER_SCALER_SET_DEFAULT;
  pdev->smudge_correct_config.nodetect_ambient_threshold =
    VL53L4CX_TUNINGPARM_DYNXTALK_NODETECT_AMB_THRESHOLD_KCPS_DEFAULT;
  pdev->smudge_correct_config.nodetect_sample_limit =
    VL53L4CX_TUNINGPARM_DYNXTALK_NODETECT_SAMPLE_LIMIT_DEFAULT;
  pdev->smudge_correct_config.nodetect_xtalk_offset =
    VL53L4CX_TUNINGPARM_DYNXTALK_NODETECT_XTALK_OFFSET_KCPS_DEFAULT;
  pdev->smudge_correct_config.nodetect_min_range_mm =
    VL53L4CX_TUNINGPARM_DYNXTALK_NODETECT_MIN_RANGE_MM_DEFAULT;
  pdev->smudge_correct_config.max_smudge_factor =
    VL53L4CX_TUNINGPARM_DYNXTALK_MAX_SMUDGE_FACTOR_DEFAULT;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_output_init(
  VL53L4CX_LLDriverResults_t *pres
)
{
  ...
  VL53L4CX_smudge_corrector_data_t *pdata;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_xtalk_cal_data_init(
  VL53L4CX_DEV                          Dev
)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_low_power_auto_data_init(
  VL53L4CX_DEV                          Dev
)
{
  ...
  pdev->low_power_auto_data.vhv_loop_bound =
    VL53L4CX_TUNINGPARM_LOWPOWERAUTO_VHV_LOOP_BOUND_DEFAULT;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_low_power_auto_data_stop_range(
  VL53L4CX_DEV                          Dev
)
{
...
)
{
  ...
  pdynamic->system__sequence_config =
    VL53L4CX_SEQUENCE_VHV_EN |
    VL53L4CX_SEQUENCE_PHASECAL_EN |
    VL53L4CX_SEQUENCE_DSS1_EN |



    VL53L4CX_SEQUENCE_RANGE_EN;


  pgeneral->dss_config__manual_effective_spads_select = 200 << 8;
  pgeneral->dss_config__roi_mode_control =
    VL53L4CX_DEVICEDSSMODE__REQUESTED_EFFFECTIVE_SPADS;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_low_power_auto_update_DSS(
  VL53L4CX_DEV        Dev)
{
  ...
  VL53L4CX_system_results_t *pS = &(pdev->sys_results);
  ...
  if (status == VL53L4CX_ERROR_DIVISION_BY_ZERO) {
...
VL53L4CX_Error VL53L4CX::VL53L4CX_compute_histo_merge_nb(
