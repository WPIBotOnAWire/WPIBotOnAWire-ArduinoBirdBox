...
#define VL53L4CX_MAX_I2C_XFER_SIZE 256

VL53L4CX_Error VL53L4CX::select_offset_per_vcsel(VL53L4CX_LLDriverData_t *pdev,
                                                 int16_t *poffset)
{
...
void VL53L4CX::vl53lx_diff_histo_stddev(VL53L4CX_LLDriverData_t *pdev,
...
void VL53L4CX::vl53lx_histo_merge(VL53L4CX_DEV Dev,
                                  VL53L4CX_histogram_bin_data_t *pdata)
{
  ...
  uint8_t    BuffSize = VL53L4CX_HISTOGRAM_BUFFER_SIZE;
  uint8_t    pos;

  VL53L4CX_get_tuning_parm(Dev, VL53L4CX_TUNINGPARM_HIST_MERGE_MAX_SIZE,
                           &TuningBinRecSize);

  VL53L4CX_get_tuning_parm(Dev, VL53L4CX_TUNINGPARM_RESET_MERGE_THRESHOLD,
                           &rmt);
    ...
    if (diff_histo_stddev >= rmt) {
      ...
      if (timing == 0)
        pdev->pos_before_next_recom =
          VL53L4CX_FRAME_WAIT_EVENT;
      else
        pdev->pos_before_next_recom =
          VL53L4CX_FRAME_WAIT_EVENT + 1;
    } else {
...
VL53L4CX_Error VL53L4CX::VL53L4CX_load_patch(VL53L4CX_DEV Dev)
{
  VL53L4CX_Error status = VL53L4CX_ERROR_NONE;
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    status = VL53L4CX_WriteMulti(Dev,
                                 VL53L4CX_PATCH__OFFSET_0, comms_buffer, 6);
  }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    comms_buffer[5] = 0x29;
    status = VL53L4CX_WriteMulti(Dev,
                                 VL53L4CX_PATCH__ADDRESS_0, comms_buffer, 6);
  }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    comms_buffer[1] = 0x07;
    status = VL53L4CX_WriteMulti(Dev,
                                 VL53L4CX_PATCH__JMP_ENABLES, comms_buffer, 2);
  }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    comms_buffer[1] = 0x07;
    status = VL53L4CX_WriteMulti(Dev,
                                 VL53L4CX_PATCH__DATA_ENABLES, comms_buffer, 2);
  }

  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_WrByte(Dev,
                             VL53L4CX_PATCH__CTRL, 0x01);

  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_WrByte(Dev,
                             VL53L4CX_FIRMWARE__ENABLE, 0x01);

  return status;
}

VL53L4CX_Error VL53L4CX::VL53L4CX_unload_patch(VL53L4CX_DEV Dev)
{
  VL53L4CX_Error status = VL53L4CX_ERROR_NONE;
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    VL53L4CX_disable_powerforce(Dev);
  }

  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_WrByte(Dev,
                             VL53L4CX_PATCH__CTRL, 0x00);
  ...
  VL53L4CX_ll_version_t *pdata)
{
  ...
  memcpy(pdata, &(pdev->version), sizeof(VL53L4CX_ll_version_t));
  ...
  uint8_t           read_p2p_data)
{
  ...
  VL53L4CX_LLDriverResults_t *pres =
    VL53L4CXDevStructGetLLResultsHandle(Dev);



  VL53L4CX_zone_objects_t    *pobjects;

  uint8_t  i = 0;

  VL53L4CX_init_ll_driver_state(
    Dev,
    VL53L4CX_DEVICESTATE_UNKNOWN);

  pres->range_results.max_results    = VL53L4CX_MAX_RANGE_RESULTS;
  pres->range_results.active_results = 0;
  pres->zone_results.max_zones       = VL53L4CX_MAX_USER_ZONES;
  pres->zone_results.active_zones    = 0;

  for (i = 0; i < VL53L4CX_MAX_USER_ZONES; i++) {
    ...
    pobjects->xmonitor.range_status =
      VL53L4CX_DEVICEERROR_NOUPDATE;
  }



  pres->zone_hists.max_zones         = VL53L4CX_MAX_USER_ZONES;
  pres->zone_hists.active_zones      = 0;



  pres->zone_cal.max_zones           = VL53L4CX_MAX_USER_ZONES;
  ...
  pdev->wait_method             = VL53L4CX_WAIT_METHOD_BLOCKING;
  pdev->preset_mode   = VL53L4CX_DEVICEPRESETMODE_HISTOGRAM_MEDIUM_RANGE;
  pdev->zone_preset             = 0;
  pdev->measurement_mode        = VL53L4CX_DEVICEMEASUREMENTMODE_STOP;

  pdev->offset_calibration_mode =
    VL53L4CX_OFFSETCALIBRATIONMODE__MM1_MM2__STANDARD;
  pdev->offset_correction_mode  =
    VL53L4CX_OFFSETCORRECTIONMODE__MM1_MM2_OFFSETS;
  pdev->dmax_mode  =
    VL53L4CX_DEVICEDMAXMODE__FMT_CAL_DATA;
  ...
  pdev->offset_results.max_results    = VL53L4CX_MAX_OFFSET_RANGE_RESULTS;
  pdev->offset_results.active_results = 0;



  pdev->gain_cal.standard_ranging_gain_factor =
    VL53L4CX_TUNINGPARM_LITE_RANGING_GAIN_FACTOR_DEFAULT;
  pdev->gain_cal.histogram_ranging_gain_factor =
    VL53L4CX_TUNINGPARM_HIST_GAIN_FACTOR_DEFAULT;
  ...
  if (read_p2p_data > 0 && status == VL53L4CX_ERROR_NONE) {
...
VL53L4CX_Error VL53L4CX::VL53L4CX_read_p2p_data(
  VL53L4CX_DEV        Dev)
{
  ...
  VL53L4CX_Error status       = VL53L4CX_ERROR_NONE;
  VL53L4CX_LLDriverData_t *pdev = VL53L4CXDevStructGetLLDriverHandle(Dev);
  VL53L4CX_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
  VL53L4CX_customer_nvm_managed_t *pN = &(pdev->customer);
  VL53L4CX_additional_offset_cal_data_t *pCD = &(pdev->add_off_cal_data);

  VL53L4CX_decoded_nvm_fmt_range_data_t fmt_rrd;

  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_get_static_nvm_managed(
               Dev,
               &(pdev->stat_nvm));

  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_get_customer_nvm_managed(
               Dev,
               &(pdev->customer));

  if (status == VL53L4CX_ERROR_NONE) {

    status = VL53L4CX_get_nvm_copy_data(
               Dev,
               &(pdev->nvm_copy_data));


    if (status == VL53L4CX_ERROR_NONE)
      VL53L4CX_copy_rtn_good_spads_to_buffer(
        &(pdev->nvm_copy_data),
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_read_nvm_optical_centre(
        Dev,
        &(pdev->optical_centre));
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_read_nvm_cal_peak_rate_map(
        Dev,
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    status =
      VL53L4CX_read_nvm_additional_offset_cal_data(
        Dev,
    ...
    if (pCD->result__mm_inner_peak_signal_count_rtn_mcps == 0 &&
      ...
      VL53L4CX_calc_mm_effective_spads(
        pdev->nvm_copy_data.roi_config__mode_roi_centre_spad,
        pdev->nvm_copy_data.roi_config__mode_roi_xy_size,
        0xC7,
        0xFF,
        &(pdev->rtn_good_spads[0]),
        VL53L4CX_RTN_SPAD_APERTURE_TRANSMISSION,
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    status =
      VL53L4CX_read_nvm_fmt_range_results_data(
        Dev,
        VL53L4CX_NVM__FMT__RANGE_RESULTS__140MM_DARK,
        &fmt_rrd);
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_RdWord(
        Dev,
        VL53L4CX_RESULT__OSC_CALIBRATE_VAL,
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_get_mode_mitigation_roi(
        Dev,
        &(pdev->mm_roi));
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_part_to_part_data(
  VL53L4CX_DEV                            Dev,
  VL53L4CX_calibration_data_t            *pcal_data)
{
  ...
  VL53L4CX_Error  status = VL53L4CX_ERROR_NONE;
  VL53L4CX_LLDriverData_t *pdev = VL53L4CXDevStructGetLLDriverHandle(Dev);
  VL53L4CX_xtalk_config_t *pC = &(pdev->xtalk_cfg);
  VL53L4CX_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
  VL53L4CX_customer_nvm_managed_t *pN = &(pdev->customer);
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    pHP->algo__crosstalk_compensation_plane_offset_kcps =
      VL53L4CX_calc_crosstalk_plane_offset_with_margin(
        pC->algo__crosstalk_compensation_plane_offset_kcps,
...
VL53L4CX_Error VL53L4CX::VL53L4CX_get_part_to_part_data(
  VL53L4CX_DEV                      Dev,
  VL53L4CX_calibration_data_t      *pcal_data)
{
  ...
  VL53L4CX_LLDriverData_t *pdev = VL53L4CXDevStructGetLLDriverHandle(Dev);
  VL53L4CX_xtalk_config_t *pC = &(pdev->xtalk_cfg);
  VL53L4CX_customer_nvm_managed_t *pCN = &(pcal_data->customer);
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_inter_measurement_period_ms(
  VL53L4CX_DEV              Dev,
  ...
  uint32_t            range_config_timeout_us)
{
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    status =
      VL53L4CX_calc_timeout_register_values(
...
VL53L4CX_Error VL53L4CX::VL53L4CX_get_timeouts_us(
  VL53L4CX_DEV           Dev,
  ...
  uint32_t      *prange_config_timeout_us)
{
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    *pmm_config_timeout_us =
      VL53L4CX_calc_decoded_timeout_us(
        timeout_encoded,
        macro_period_us);
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_user_zone(
  VL53L4CX_DEV              Dev,
  VL53L4CX_user_zone_t     *puser_zone)
{
  ...
  VL53L4CX_encode_row_col(
    puser_zone->y_centre,
    puser_zone->x_centre,
    &(pdev->dyn_cfg.roi_config__user_roi_centre_spad));


  VL53L4CX_encode_zone_size(
    puser_zone->width,
    puser_zone->height,
  ...
  VL53L4CX_user_zone_t     *puser_zone)
{
  ...
  VL53L4CX_decode_row_col(
    pdev->dyn_cfg.roi_config__user_roi_centre_spad,
    &(puser_zone->y_centre),
    &(puser_zone->x_centre));


  VL53L4CX_decode_zone_size(
    pdev->dyn_cfg.roi_config__user_roi_requested_global_xy_size,
    &(puser_zone->width),
    &(puser_zone->height));
...
VL53L4CX_Error VL53L4CX::VL53L4CX_init_zone_config_histogram_bins(
  VL53L4CX_zone_config_t   *pdata)
{
  ...
  for (i = 0; i < pdata->max_zones; i++) {
    pdata->bin_config[i] = VL53L4CX_ZONECONFIG_BINCONFIG__LOWAMB;
  }

  return status;
}

VL53L4CX_Error VL53L4CX::VL53L4CX_set_zone_config(
  VL53L4CX_DEV                 Dev,
  VL53L4CX_zone_config_t      *pzone_cfg)
{
  ...
  status = VL53L4CX_init_zone_config_histogram_bins(&pdev->zone_cfg);



  if (pzone_cfg->active_zones == 0) {
    pdev->gen_cfg.global_config__stream_divider = 0;
  } else if (pzone_cfg->active_zones < VL53L4CX_MAX_USER_ZONES)
    pdev->gen_cfg.global_config__stream_divider =
      pzone_cfg->active_zones + 1;
  else
    pdev->gen_cfg.global_config__stream_divider =
      VL53L4CX_MAX_USER_ZONES + 1;

  return status;
}


VL53L4CX_Error VL53L4CX::VL53L4CX_get_zone_config(
  VL53L4CX_DEV                 Dev,
  VL53L4CX_zone_config_t      *pzone_cfg)
{
  ...
  memcpy(pzone_cfg, &(pdev->zone_cfg), sizeof(VL53L4CX_zone_config_t));

  return status;
}


VL53L4CX_Error VL53L4CX::VL53L4CX_get_preset_mode_timing_cfg(
  VL53L4CX_DEV                   Dev,
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_preset_mode(
  VL53L4CX_DEV                   Dev,
  VL53L4CX_DevicePresetModes     device_preset_mode,
  ...
  uint32_t                     inter_measurement_period_ms)
{
  ...
  VL53L4CX_static_config_t        *pstatic       = &(pdev->stat_cfg);
  VL53L4CX_histogram_config_t     *phistogram    = &(pdev->hist_cfg);
  VL53L4CX_general_config_t       *pgeneral      = &(pdev->gen_cfg);
  VL53L4CX_timing_config_t        *ptiming       = &(pdev->tim_cfg);
  VL53L4CX_dynamic_config_t       *pdynamic      = &(pdev->dyn_cfg);
  VL53L4CX_system_control_t       *psystem       = &(pdev->sys_ctrl);
  VL53L4CX_zone_config_t          *pzone_cfg     = &(pdev->zone_cfg);
  VL53L4CX_tuning_parm_storage_t  *ptuning_parms = &(pdev->tuning_parms);
  ...
  switch (device_preset_mode) {
    case VL53L4CX_DEVICEPRESETMODE_HISTOGRAM_LONG_RANGE:
      ...
      status = VL53L4CX_preset_mode_histogram_long_range(
                 phistpostprocess,
                 pstatic,
                 phistogram,
                 pgeneral,
    ...
    case VL53L4CX_DEVICEPRESETMODE_HISTOGRAM_MEDIUM_RANGE:
      status = VL53L4CX_preset_mode_histogram_medium_range(
                 phistpostprocess,
                 pstatic,
                 phistogram,
                 pgeneral,
    ...
    case VL53L4CX_DEVICEPRESETMODE_HISTOGRAM_SHORT_RANGE:
      status = VL53L4CX_preset_mode_histogram_short_range(
                 phistpostprocess,
                 pstatic,
                 phistogram,
                 pgeneral,
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_set_inter_measurement_period_ms(
        Dev,
        inter_measurement_period_ms);
...
VL53L4CX_Error  VL53L4CX::VL53L4CX_enable_xtalk_compensation(
  VL53L4CX_DEV                 Dev)
{
  ...
  tempu32 = VL53L4CX_calc_crosstalk_plane_offset_with_margin(
              pC->algo__crosstalk_compensation_plane_offset_kcps,
              pC->lite_mode_crosstalk_margin_kcps);
  if (tempu32 > 0xFFFF) {
    tempu32 = 0xFFFF;
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    pC->crosstalk_range_ignore_threshold_rate_mcps =
      VL53L4CX_calc_range_ignore_threshold(
        pC->algo__crosstalk_compensation_plane_offset_kcps,
        pC->algo__crosstalk_compensation_x_plane_gradient_kcps,
        pC->algo__crosstalk_compensation_y_plane_gradient_kcps,
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_set_customer_nvm_managed(
        Dev,
        &(pdev->customer));
...
void VL53L4CX::VL53L4CX_get_xtalk_compensation_enable(
  VL53L4CX_DEV    Dev,
...
VL53L4CX_Error  VL53L4CX::VL53L4CX_disable_xtalk_compensation(
  VL53L4CX_DEV                 Dev)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_init_and_start_range(
  VL53L4CX_DEV                     Dev,
  ...
  VL53L4CX_DeviceConfigLevel       device_config_level)
{
  ...
  uint8_t buffer[VL53L4CX_MAX_I2C_XFER_SIZE];

  VL53L4CX_static_nvm_managed_t   *pstatic_nvm   = &(pdev->stat_nvm);
  VL53L4CX_customer_nvm_managed_t *pcustomer_nvm = &(pdev->customer);
  VL53L4CX_static_config_t        *pstatic       = &(pdev->stat_cfg);
  VL53L4CX_general_config_t       *pgeneral      = &(pdev->gen_cfg);
  VL53L4CX_timing_config_t        *ptiming       = &(pdev->tim_cfg);
  VL53L4CX_dynamic_config_t       *pdynamic      = &(pdev->dyn_cfg);
  ...
  VL53L4CX_ll_driver_state_t  *pstate   = &(pdev->ll_state);
  VL53L4CX_customer_nvm_managed_t *pN = &(pdev->customer);
  ...
  psystem->system__mode_start =
    (psystem->system__mode_start &
     VL53L4CX_DEVICEMEASUREMENTMODE_STOP_MASK) |
    measurement_mode;



  status =
    VL53L4CX_set_user_zone(
      Dev,
  ...
  if (pdev->zone_cfg.active_zones > 0) {
    status =
      VL53L4CX_set_zone_dss_config(
        Dev,
        &(pres->zone_dyn_cfgs.VL53L4CX_p_003[pdev->ll_state.cfg_zone_id])
      );
  }
  ...
  if (((pdev->sys_ctrl.system__mode_start &
        VL53L4CX_DEVICESCHEDULERMODE_HISTOGRAM) == 0x00) &&
      (pdev->xtalk_cfg.global_crosstalk_compensation_enable
       == 0x01)) {
  ...
  if ((pdev->low_power_auto_data.is_low_power_auto_mode == 1) &&
    ...
    if ((pdev->dyn_cfg.system__sequence_config & (
           VL53L4CX_SEQUENCE_MM1_EN | VL53L4CX_SEQUENCE_MM2_EN)) ==
        0x0) {
    ...
    if (device_config_level <
        VL53L4CX_DEVICECONFIGLEVEL_CUSTOMER_ONWARDS) {
      device_config_level =
        VL53L4CX_DEVICECONFIGLEVEL_CUSTOMER_ONWARDS;
    }
  }
  ...
  if ((pdev->low_power_auto_data.is_low_power_auto_mode == 1) &&
    ...
    device_config_level = VL53L4CX_DEVICECONFIGLEVEL_FULL;
  }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    status = VL53L4CX_save_cfg_data(Dev);
  }



  switch (device_config_level) {
    case VL53L4CX_DEVICECONFIGLEVEL_FULL:
      i2c_index = VL53L4CX_STATIC_NVM_MANAGED_I2C_INDEX;
      break;
    case VL53L4CX_DEVICECONFIGLEVEL_CUSTOMER_ONWARDS:
      i2c_index = VL53L4CX_CUSTOMER_NVM_MANAGED_I2C_INDEX;
      break;
    case VL53L4CX_DEVICECONFIGLEVEL_STATIC_ONWARDS:
      i2c_index = VL53L4CX_STATIC_CONFIG_I2C_INDEX;
      break;
    case VL53L4CX_DEVICECONFIGLEVEL_GENERAL_ONWARDS:
      i2c_index = VL53L4CX_GENERAL_CONFIG_I2C_INDEX;
      break;
    case VL53L4CX_DEVICECONFIGLEVEL_TIMING_ONWARDS:
      i2c_index = VL53L4CX_TIMING_CONFIG_I2C_INDEX;
      break;
    case VL53L4CX_DEVICECONFIGLEVEL_DYNAMIC_ONWARDS:
      i2c_index = VL53L4CX_DYNAMIC_CONFIG_I2C_INDEX;
      break;
    default:
      i2c_index = VL53L4CX_SYSTEM_CONTROL_I2C_INDEX;
      break;
  }
  ...
  i2c_buffer_size_bytes =
    (VL53L4CX_SYSTEM_CONTROL_I2C_INDEX +
     VL53L4CX_SYSTEM_CONTROL_I2C_SIZE_BYTES) -
    i2c_index;
  ...
  if (device_config_level >= VL53L4CX_DEVICECONFIGLEVEL_FULL &&
      status == VL53L4CX_ERROR_NONE) {

    i2c_buffer_offset_bytes =
      VL53L4CX_STATIC_NVM_MANAGED_I2C_INDEX - i2c_index;

    status =
      VL53L4CX_i2c_encode_static_nvm_managed(
        pstatic_nvm,
        VL53L4CX_STATIC_NVM_MANAGED_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes]);
  }

  if (device_config_level >= VL53L4CX_DEVICECONFIGLEVEL_CUSTOMER_ONWARDS &&
      status == VL53L4CX_ERROR_NONE) {

    i2c_buffer_offset_bytes =
      VL53L4CX_CUSTOMER_NVM_MANAGED_I2C_INDEX - i2c_index;

    status =
      VL53L4CX_i2c_encode_customer_nvm_managed(
        pcustomer_nvm,
        VL53L4CX_CUSTOMER_NVM_MANAGED_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes]);
  }

  if (device_config_level >= VL53L4CX_DEVICECONFIGLEVEL_STATIC_ONWARDS &&
      status == VL53L4CX_ERROR_NONE) {

    i2c_buffer_offset_bytes =
      VL53L4CX_STATIC_CONFIG_I2C_INDEX - i2c_index;

    status =
      VL53L4CX_i2c_encode_static_config(
        pstatic,
        VL53L4CX_STATIC_CONFIG_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes]);
  }

  if (device_config_level >= VL53L4CX_DEVICECONFIGLEVEL_GENERAL_ONWARDS &&
      status == VL53L4CX_ERROR_NONE) {

    i2c_buffer_offset_bytes =
      VL53L4CX_GENERAL_CONFIG_I2C_INDEX - i2c_index;

    status =
      VL53L4CX_i2c_encode_general_config(
        pgeneral,
        VL53L4CX_GENERAL_CONFIG_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes]);
  }

  if (device_config_level >= VL53L4CX_DEVICECONFIGLEVEL_TIMING_ONWARDS &&
      status == VL53L4CX_ERROR_NONE) {

    i2c_buffer_offset_bytes =
      VL53L4CX_TIMING_CONFIG_I2C_INDEX - i2c_index;

    status =
      VL53L4CX_i2c_encode_timing_config(
        ptiming,
        VL53L4CX_TIMING_CONFIG_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes]);
  }

  if (device_config_level >= VL53L4CX_DEVICECONFIGLEVEL_DYNAMIC_ONWARDS &&
      status == VL53L4CX_ERROR_NONE) {

    i2c_buffer_offset_bytes =
      VL53L4CX_DYNAMIC_CONFIG_I2C_INDEX - i2c_index;


    if ((psystem->system__mode_start &
         VL53L4CX_DEVICEMEASUREMENTMODE_BACKTOBACK) ==
        VL53L4CX_DEVICEMEASUREMENTMODE_BACKTOBACK) {
    ...
    status =
      VL53L4CX_i2c_encode_dynamic_config(
        pdynamic,
        VL53L4CX_DYNAMIC_CONFIG_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes]);
  }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    i2c_buffer_offset_bytes =
      VL53L4CX_SYSTEM_CONTROL_I2C_INDEX - i2c_index;

    status =
      VL53L4CX_i2c_encode_system_control(
        psystem,
        VL53L4CX_SYSTEM_CONTROL_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes]);
  }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    status = VL53L4CX_update_ll_driver_rd_state(Dev);
  }

  if (status == VL53L4CX_ERROR_NONE) {
    status = VL53L4CX_update_ll_driver_cfg_state(Dev);
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_stop_range(
  VL53L4CX_DEV     Dev)
{
  ...
  pdev->sys_ctrl.system__mode_start =
    (pdev->sys_ctrl.system__mode_start &
     VL53L4CX_DEVICEMEASUREMENTMODE_STOP_MASK) |
    VL53L4CX_DEVICEMEASUREMENTMODE_ABORT;

  status = VL53L4CX_set_system_control(
             Dev,
             &pdev->sys_ctrl);
  ...
  if (pdev->low_power_auto_data.is_low_power_auto_mode == 1) {
    VL53L4CX_low_power_auto_data_stop_range(Dev);
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_get_measurement_results(
  VL53L4CX_DEV                     Dev,
  VL53L4CX_DeviceResultsLevel      device_results_level)
{
  ...
  VL53L4CX_system_results_t   *psystem_results = &(pdev->sys_results);
  VL53L4CX_core_results_t     *pcore_results   = &(pdev->core_results);
  VL53L4CX_debug_results_t    *pdebug_results  = &(pdev->dbg_results);

  uint16_t i2c_index               = VL53L4CX_SYSTEM_RESULTS_I2C_INDEX;
  uint16_t i2c_buffer_offset_bytes = 0;
  uint16_t i2c_buffer_size_bytes   = 0;

  switch (device_results_level) {
    case VL53L4CX_DEVICERESULTSLEVEL_FULL:
      i2c_buffer_size_bytes =
        (VL53L4CX_DEBUG_RESULTS_I2C_INDEX +
         VL53L4CX_DEBUG_RESULTS_I2C_SIZE_BYTES) -
        i2c_index;
      break;
    case VL53L4CX_DEVICERESULTSLEVEL_UPTO_CORE:
      i2c_buffer_size_bytes =
        (VL53L4CX_CORE_RESULTS_I2C_INDEX +
         VL53L4CX_CORE_RESULTS_I2C_SIZE_BYTES) -
        i2c_index;
      break;
    default:
      i2c_buffer_size_bytes =
        VL53L4CX_SYSTEM_RESULTS_I2C_SIZE_BYTES;
      break;
  }
  ...
  if (device_results_level >= VL53L4CX_DEVICERESULTSLEVEL_FULL &&
      status == VL53L4CX_ERROR_NONE) {

    i2c_buffer_offset_bytes =
      VL53L4CX_DEBUG_RESULTS_I2C_INDEX - i2c_index;

    status =
      VL53L4CX_i2c_decode_debug_results(
        VL53L4CX_DEBUG_RESULTS_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes],
        pdebug_results);
  }

  if (device_results_level >= VL53L4CX_DEVICERESULTSLEVEL_UPTO_CORE &&
      status == VL53L4CX_ERROR_NONE) {

    i2c_buffer_offset_bytes =
      VL53L4CX_CORE_RESULTS_I2C_INDEX - i2c_index;

    status =
      VL53L4CX_i2c_decode_core_results(
        VL53L4CX_CORE_RESULTS_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes],
        pcore_results);
  }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    status =
      VL53L4CX_i2c_decode_system_results(
        VL53L4CX_SYSTEM_RESULTS_I2C_SIZE_BYTES,
        &buffer[i2c_buffer_offset_bytes],
        psystem_results);
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_get_device_results(
  VL53L4CX_DEV                    Dev,
  VL53L4CX_DeviceResultsLevel     device_results_level,
  VL53L4CX_range_results_t       *prange_results)
{
  ...
  VL53L4CX_range_results_t   *presults =
    &(pres->range_results);
  VL53L4CX_zone_objects_t    *pobjects =
    &(pres->zone_results.VL53L4CX_p_003[0]);
  VL53L4CX_ll_driver_state_t *pstate   =
    &(pdev->ll_state);
  VL53L4CX_zone_config_t     *pzone_cfg =
    &(pdev->zone_cfg);
  VL53L4CX_zone_hist_info_t  *phist_info =
    &(pres->zone_hists.VL53L4CX_p_003[0]);

  VL53L4CX_dmax_calibration_data_t   dmax_cal;
  VL53L4CX_dmax_calibration_data_t *pdmax_cal = &dmax_cal;
  VL53L4CX_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
  VL53L4CX_xtalk_config_t *pC = &(pdev->xtalk_cfg);
  VL53L4CX_low_power_auto_data_t *pL = &(pdev->low_power_auto_data);
  VL53L4CX_histogram_bin_data_t *pHD = &(pdev->hist_data);
  VL53L4CX_customer_nvm_managed_t *pN = &(pdev->customer);
  VL53L4CX_zone_histograms_t *pZH = &(pres->zone_hists);
  VL53L4CX_xtalk_calibration_results_t *pXCR = &(pdev->xtalk_cal);
  uint8_t tmp8;
  uint8_t zid;
  uint8_t i;
  ...
  if ((pdev->sys_ctrl.system__mode_start &
       VL53L4CX_DEVICESCHEDULERMODE_HISTOGRAM)
      == VL53L4CX_DEVICESCHEDULERMODE_HISTOGRAM) {



    status = VL53L4CX_get_histogram_bin_data(
               Dev,
               &(pdev->hist_data));
    ...
    if (status == VL53L4CX_ERROR_NONE &&
      ...
      status = VL53L4CX_hist_copy_and_scale_ambient_info(
                 &(pZH->VL53L4CX_p_003[zid]),
                 &(pdev->hist_data));
    }
    ...
    VL53L4CX_compute_histo_merge_nb(Dev, &histo_merge_nb);
    if (histo_merge_nb == 0) {
      histo_merge_nb = 1;
    }
    ...
    switch (pdev->offset_correction_mode) {
      ...
      case VL53L4CX_OFFSETCORRECTIONMODE__MM1_MM2_OFFSETS:
        tmp8 = pdev->gen_cfg.dss_config__aperture_attenuation;

        VL53L4CX_hist_combine_mm1_mm2_offsets(
          ...
          &(pHP->range_offset_mm));
        break;
      case VL53L4CX_OFFSETCORRECTIONMODE__PER_VCSEL_OFFSETS:
    ...
    VL53L4CX_calc_max_effective_spads(
      pHD->roi_config__user_roi_centre_spad,
      pHD->roi_config__user_roi_requested_global_xy_size,
      &(pdev->rtn_good_spads[0]),
    ...
    status =
      VL53L4CX_get_dmax_calibration_data(
        Dev,
        pdev->dmax_mode,
        pdmax_cal);
    ...
    status = VL53L4CX_ipp_hist_process_data(
               Dev,
               pdmax_cal,
               &(pdev->dmax_cfg),
               &(pdev->histpostprocess),
    ...
    if ((pdev->tuning_parms.tp_hist_merge == 1) &&
      ...
      for (i = 0; i < VL53L4CX_MAX_RANGE_RESULTS; i++) {
        pdata = &(presults->VL53L4CX_p_003[i]);
        pdata->VL53L4CX_p_016 /= histo_merge_nb;
        pdata->VL53L4CX_p_017 /= histo_merge_nb;
        pdata->VL53L4CX_p_010 /= histo_merge_nb;
        ...
        pdata->VL53L4CX_p_009 /= histo_merge_nb;
      }
    ...
    status = VL53L4CX_hist_wrap_dmax(
               &(pdev->histpostprocess),
               &(pdev->hist_data),
    ...
    zid = pdev->ll_state.rd_zone_id;
    status = VL53L4CX_hist_phase_consistency_check(
               Dev,
               &(pZH->VL53L4CX_p_003[zid]),
               &(pres->zone_results.VL53L4CX_p_003[zid]),
               presults);
    ...
    zid = pdev->ll_state.rd_zone_id;
    status = VL53L4CX_hist_xmonitor_consistency_check(
               Dev,
               &(pZH->VL53L4CX_p_003[zid]),
               &(pres->zone_results.VL53L4CX_p_003[zid]),
               &(presults->xmonitor));
    ...
    zid = pdev->ll_state.rd_zone_id;
    pZH->max_zones    = VL53L4CX_MAX_USER_ZONES;
    pZH->active_zones =
      pdev->zone_cfg.active_zones + 1;
    pHD->zone_id       = zid;
    ...
    VL53L4CX_hist_copy_results_to_sys_and_core(
      &(pdev->hist_data),
      presults,
      &(pdev->sys_results),
      &(pdev->core_results));
...
UPDATE_DYNAMIC_CONFIG:
    if (pzone_cfg->active_zones > 0) {
      if (pstate->rd_device_state !=
          VL53L4CX_DEVICESTATE_RANGING_WAIT_GPH_SYNC) {
        if (status == VL53L4CX_ERROR_NONE) {
          status = VL53L4CX_dynamic_zone_update(
                     Dev, presults);
        }
      }
      ...
      for (i = 0; i < VL53L4CX_MAX_USER_ZONES; i++) {
        pzone_cfg->bin_config[i] =
          ((pdev->ll_state.cfg_internal_stream_count)
           & 0x01) ?
          VL53L4CX_ZONECONFIG_BINCONFIG__HIGHAMB :
          VL53L4CX_ZONECONFIG_BINCONFIG__LOWAMB;
      }

      if (status == VL53L4CX_ERROR_NONE) {
        status = VL53L4CX_multizone_hist_bins_update(Dev);
      }
    ...
    if (status == VL53L4CX_ERROR_NONE) {
      status = VL53L4CX_dynamic_xtalk_correction_corrector(Dev);
    }
    ...
    if (status == VL53L4CX_ERROR_NONE)
      status = VL53L4CX_get_measurement_results(
                 Dev,
                 device_results_level);

    if (status == VL53L4CX_ERROR_NONE)
      VL53L4CX_copy_sys_and_core_results_to_range_results(
    ...
    if (pL->is_low_power_auto_mode == 1) {
      ...
      if ((status == VL53L4CX_ERROR_NONE) &&
        ...
        status =
          VL53L4CX_low_power_auto_setup_manual_calibration(
            Dev);
        pL->low_power_auto_range_count = 1;
      } else if ((status == VL53L4CX_ERROR_NONE) &&
                 (pL->low_power_auto_range_count == 1)) {
      ...
      if ((pL->low_power_auto_range_count != 0xFF) &&
          (status == VL53L4CX_ERROR_NONE)) {
        status = VL53L4CX_low_power_auto_update_DSS(
                   Dev);
      }
    }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    if (zid < pres->zone_results.max_zones) {
      ...
      pobjects->xmonitor.VL53L4CX_p_016 =
        presults->xmonitor.VL53L4CX_p_016;
      pobjects->xmonitor.VL53L4CX_p_017 =
        presults->xmonitor.VL53L4CX_p_017;
      pobjects->xmonitor.VL53L4CX_p_011 =
        presults->xmonitor.VL53L4CX_p_011;
      pobjects->xmonitor.range_status =
      ...
      for (i = 0; i < presults->active_results; i++) {
        pobjects->VL53L4CX_p_003[i].VL53L4CX_p_016 =
          presults->VL53L4CX_p_003[i].VL53L4CX_p_016;
        pobjects->VL53L4CX_p_003[i].VL53L4CX_p_017 =
          presults->VL53L4CX_p_003[i].VL53L4CX_p_017;
        pobjects->VL53L4CX_p_003[i].VL53L4CX_p_011 =
          presults->VL53L4CX_p_003[i].VL53L4CX_p_011;
        pobjects->VL53L4CX_p_003[i].range_status =
          presults->VL53L4CX_p_003[i].range_status;
      }
  ...
  memcpy(
    prange_results,
    presults,
    sizeof(VL53L4CX_range_results_t));



  if (status == VL53L4CX_ERROR_NONE) {
    status = VL53L4CX_check_ll_driver_rd_state(Dev);
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_clear_interrupt_and_enable_next_range(
  VL53L4CX_DEV        Dev,
  uint8_t           measurement_mode)
{
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_init_and_start_range(
               Dev,
               measurement_mode,
               VL53L4CX_DEVICECONFIGLEVEL_GENERAL_ONWARDS);

  return status;
}


VL53L4CX_Error VL53L4CX::VL53L4CX_get_histogram_bin_data(
  VL53L4CX_DEV                   Dev,
  VL53L4CX_histogram_bin_data_t *pdata)
{
  ...
  VL53L4CX_static_nvm_managed_t   *pstat_nvm = &(pdev->stat_nvm);
  VL53L4CX_static_config_t        *pstat_cfg = &(pdev->stat_cfg);
  VL53L4CX_general_config_t       *pgen_cfg  = &(pdev->gen_cfg);
  VL53L4CX_timing_config_t        *ptim_cfg  = &(pdev->tim_cfg);
  VL53L4CX_range_results_t        *presults  = &(pres->range_results);
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_ReadMulti(
               Dev,
               VL53L4CX_HISTOGRAM_BIN_DATA_I2C_INDEX,
               pbuffer,
               VL53L4CX_HISTOGRAM_BIN_DATA_I2C_SIZE_BYTES);
  ...
  pdata->result__dss_actual_effective_spads =
    VL53L4CX_i2c_decode_uint16_t(2, pbuffer +   4);



  i2c_buffer_offset_bytes =
    VL53L4CX_PHASECAL_RESULT__REFERENCE_PHASE -
    VL53L4CX_HISTOGRAM_BIN_DATA_I2C_INDEX;

  pbuffer = &buffer[i2c_buffer_offset_bytes];

  pdata->phasecal_result__reference_phase =
    VL53L4CX_i2c_decode_uint16_t(2, pbuffer);

  i2c_buffer_offset_bytes =
    VL53L4CX_PHASECAL_RESULT__VCSEL_START -
    VL53L4CX_HISTOGRAM_BIN_DATA_I2C_INDEX;
  ...
  i2c_buffer_offset_bytes =
    VL53L4CX_RESULT__HISTOGRAM_BIN_23_0_MSB -
    VL53L4CX_HISTOGRAM_BIN_DATA_I2C_INDEX;

  bin_23_0 = buffer[i2c_buffer_offset_bytes] << 2;

  i2c_buffer_offset_bytes =
    VL53L4CX_RESULT__HISTOGRAM_BIN_23_0_LSB -
    VL53L4CX_HISTOGRAM_BIN_DATA_I2C_INDEX;

  bin_23_0 += buffer[i2c_buffer_offset_bytes];

  i2c_buffer_offset_bytes =
    VL53L4CX_RESULT__HISTOGRAM_BIN_23_0 -
    VL53L4CX_HISTOGRAM_BIN_DATA_I2C_INDEX;
  ...
  i2c_buffer_offset_bytes =
    VL53L4CX_RESULT__HISTOGRAM_BIN_0_2 -
    VL53L4CX_HISTOGRAM_BIN_DATA_I2C_INDEX;

  pbuffer = &buffer[i2c_buffer_offset_bytes];
  for (bin = 0; bin < VL53L4CX_HISTOGRAM_BUFFER_SIZE; bin++) {
    pdata->bin_data[bin] =
      (int32_t)VL53L4CX_i2c_decode_uint32_t(3, pbuffer);
    pbuffer += 3;
  }
  ...
  VL53L4CX_get_tuning_parm(Dev, VL53L4CX_TUNINGPARM_HIST_MERGE, &hist_merge);
  ...
  pdata->VL53L4CX_p_019               = 0;
  pdata->VL53L4CX_p_020             = VL53L4CX_HISTOGRAM_BUFFER_SIZE;
  pdata->VL53L4CX_p_021          = VL53L4CX_HISTOGRAM_BUFFER_SIZE;
  ...
  pdata->VL53L4CX_p_015 =
    pstat_nvm->osc_measured__fast_osc__frequency;



  VL53L4CX_hist_get_bin_sequence_config(Dev, pdata);
  ...
  if (pdev->ll_state.rd_timing_status == 0) {
    ...
    pdata->VL53L4CX_p_005 =  ptim_cfg->range_config__vcsel_period_a;
  } else {

    encoded_timeout =
      (ptim_cfg->range_config__timeout_macrop_b_hi << 8)
      + ptim_cfg->range_config__timeout_macrop_b_lo;
    pdata->VL53L4CX_p_005 = ptim_cfg->range_config__vcsel_period_b;
  }
  ...
  pdata->total_periods_elapsed =
    VL53L4CX_decode_timeout(encoded_timeout);




  pll_period_us =
    VL53L4CX_calc_pll_period_us(pdata->VL53L4CX_p_015);
  ...
  VL53L4CX_hist_calc_zero_distance_phase(pdata);



  VL53L4CX_hist_estimate_ambient_from_ambient_bins(pdata);
  ...
  pzone_dyn_cfg = &(pres->zone_dyn_cfgs.VL53L4CX_p_003[pdata->zone_id]);
  ...
  presults->device_status = VL53L4CX_DEVICEERROR_NOUPDATE;



  switch (pdata->result__range_status &
          VL53L4CX_RANGE_STATUS__RANGE_STATUS_MASK) {
    ...
    case VL53L4CX_DEVICEERROR_MULTCLIPFAIL:
      ...
      status = VL53L4CX_ERROR_RANGE_ERROR;
...
void VL53L4CX::VL53L4CX_copy_sys_and_core_results_to_range_results(
  int32_t                           gain_factor,
  ...
  VL53L4CX_range_results_t           *presults)
{
  ...
  for (i = 0; i < VL53L4CX_MAX_AMBIENT_DMAX_VALUES; i++) {
    presults->VL53L4CX_p_022[i] = 0;
  }
  ...
  for (i = 0; i < 2; i++) {
    ...
    if ((psys->result__stream_count == 0) &&
        ((psys->result__range_status &
          VL53L4CX_RANGE_STATUS__RANGE_STATUS_MASK) ==
         VL53L4CX_DEVICEERROR_RANGECOMPLETE)) {
      pdata->range_status =
        VL53L4CX_DEVICEERROR_RANGECOMPLETE_NO_WRAP_CHECK;
    } else {
      pdata->range_status =
        psys->result__range_status &
        VL53L4CX_RANGE_STATUS__RANGE_STATUS_MASK;
    ...
    pdata->VL53L4CX_p_012 = 0;
    pdata->VL53L4CX_p_019    = 0;
    pdata->VL53L4CX_p_023   = 0;
    pdata->VL53L4CX_p_024     = 0;
    pdata->VL53L4CX_p_013   = 0;
    pdata->VL53L4CX_p_025    = 0;

    switch (i) {

      case 0:
        if (psys->result__report_status ==
            VL53L4CX_DEVICEREPORTSTATUS_MM1)
          pdata->VL53L4CX_p_004 =
            rmmi_effective_spads_sd0;
        else if (psys->result__report_status ==
                 VL53L4CX_DEVICEREPORTSTATUS_MM2)
          pdata->VL53L4CX_p_004 =
            rmmo_effective_spads_sd0;
        else
          pdata->VL53L4CX_p_004 =
        ...
        pdata->VL53L4CX_p_002 = (uint16_t)tmpu32;
  ...
  switch (psys->result__range_status &
    ...
    case VL53L4CX_DEVICEERROR_MULTCLIPFAIL:
      ...
      presults->VL53L4CX_p_003[0].range_status =
        VL53L4CX_DEVICEERROR_NOUPDATE;
      break;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_zone_dss_config(
  VL53L4CX_DEV                      Dev,
  VL53L4CX_zone_private_dyn_cfg_t  *pzone_dyn_cfg)
{
  ...
  if (pstate->cfg_device_state ==
      VL53L4CX_DEVICESTATE_RANGING_DSS_MANUAL) {
    pdev->gen_cfg.dss_config__roi_mode_control =
      VL53L4CX_DSS_CONTROL__MODE_EFFSPADS;
    pdev->gen_cfg.dss_config__manual_effective_spads_select =
      pzone_dyn_cfg->dss_requested_effective_spad_count;
  } else {
    pdev->gen_cfg.dss_config__roi_mode_control =
      VL53L4CX_DSS_CONTROL__MODE_TARGET_RATE;
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_dmax_mode(
  VL53L4CX_DEV               Dev,
  ...
  VL53L4CX_dmax_calibration_data_t *pdmax_cal)
{
  ...
  switch (dmax_mode) {
    ...
    case VL53L4CX_DEVICEDMAXMODE__CUST_CAL_DATA:
      memcpy(
        pdmax_cal,
        &(pdev->cust_dmax_cal),
        sizeof(VL53L4CX_dmax_calibration_data_t));
    ...
    case VL53L4CX_DEVICEDMAXMODE__FMT_CAL_DATA:
      memcpy(
        pdmax_cal,
        &(pdev->fmt_dmax_cal),
        sizeof(VL53L4CX_dmax_calibration_data_t));
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_offset_correction_mode(
  VL53L4CX_DEV                     Dev,
  ...
  VL53L4CX_tuning_parameters_t           *ptun_data)
{
  ...
  VL53L4CX_LLDriverData_t *pdev = VL53L4CXDevStructGetLLDriverHandle(Dev);
  VL53L4CX_hist_post_process_config_t *pHP = &(pdev->histpostprocess);
  VL53L4CX_xtalkextract_config_t *pXC = &(pdev->xtalk_extract_cfg);
  ...
  ptun_data->vl53lx_tuningparm_refspadchar_vcsel_period =
    pdev->refspadchar.VL53L4CX_p_005;
  ...
  int32_t                       *ptuning_parm_value)
{
  ...
  switch (tuning_parm_key) {
    ...
    case VL53L4CX_TUNINGPARM_VERSION:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_tuning_parm_version;
      break;
    case VL53L4CX_TUNINGPARM_KEY_TABLE_VERSION:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_tuning_parm_key_table_version;
      break;
    case VL53L4CX_TUNINGPARM_LLD_VERSION:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_tuning_parm_lld_version;
      break;
    case VL53L4CX_TUNINGPARM_HIST_ALGO_SELECT:
      *ptuning_parm_value =
        (int32_t)pHP->hist_algo_select;
      break;
    case VL53L4CX_TUNINGPARM_HIST_TARGET_ORDER:
      *ptuning_parm_value =
        (int32_t)pHP->hist_target_order;
      break;
    case VL53L4CX_TUNINGPARM_HIST_FILTER_WOI_0:
      *ptuning_parm_value =
        (int32_t)pHP->filter_woi0;
      break;
    case VL53L4CX_TUNINGPARM_HIST_FILTER_WOI_1:
      *ptuning_parm_value =
        (int32_t)pHP->filter_woi1;
      break;
    case VL53L4CX_TUNINGPARM_HIST_AMB_EST_METHOD:
      *ptuning_parm_value =
        (int32_t)pHP->hist_amb_est_method;
      break;
    case VL53L4CX_TUNINGPARM_HIST_AMB_THRESH_SIGMA_0:
      *ptuning_parm_value =
        (int32_t)pHP->ambient_thresh_sigma0;
      break;
    case VL53L4CX_TUNINGPARM_HIST_AMB_THRESH_SIGMA_1:
      *ptuning_parm_value =
        (int32_t)pHP->ambient_thresh_sigma1;
      break;
    case VL53L4CX_TUNINGPARM_HIST_MIN_AMB_THRESH_EVENTS:
      *ptuning_parm_value =
        (int32_t)pHP->min_ambient_thresh_events;
      break;
    case VL53L4CX_TUNINGPARM_HIST_AMB_EVENTS_SCALER:
      *ptuning_parm_value =
        (int32_t)pHP->ambient_thresh_events_scaler;
      break;
    case VL53L4CX_TUNINGPARM_HIST_NOISE_THRESHOLD:
      *ptuning_parm_value =
        (int32_t)pHP->noise_threshold;
      break;
    case VL53L4CX_TUNINGPARM_HIST_SIGNAL_TOTAL_EVENTS_LIMIT:
      *ptuning_parm_value =
        (int32_t)pHP->signal_total_events_limit;
      break;
    case VL53L4CX_TUNINGPARM_HIST_SIGMA_EST_REF_MM:
      *ptuning_parm_value =
        (int32_t)pHP->sigma_estimator__sigma_ref_mm;
      break;
    case VL53L4CX_TUNINGPARM_HIST_SIGMA_THRESH_MM:
      *ptuning_parm_value =
        (int32_t)pHP->sigma_thresh;
      break;
    case VL53L4CX_TUNINGPARM_HIST_GAIN_FACTOR:
      *ptuning_parm_value =
        (int32_t)pdev->gain_cal.histogram_ranging_gain_factor;
      break;
    case VL53L4CX_TUNINGPARM_CONSISTENCY_HIST_PHASE_TOLERANCE:
      *ptuning_parm_value =
        (int32_t)pHP->algo__consistency_check__phase_tolerance;
      break;
    case VL53L4CX_TUNINGPARM_CONSISTENCY_HIST_MIN_MAX_TOLERANCE_MM:
      *ptuning_parm_value =
        (int32_t)pHP->algo__consistency_check__min_max_tolerance;
      break;
    case VL53L4CX_TUNINGPARM_CONSISTENCY_HIST_EVENT_SIGMA:
      *ptuning_parm_value =
        (int32_t)pHP->algo__consistency_check__event_sigma;
      break;
    case VL53L4CX_TUNINGPARM_CONSISTENCY_HIST_EVENT_SIGMA_MIN_SPAD_LIMIT:
      *ptuning_parm_value =
        (int32_t)pHP->algo__consistency_check__event_min_spad_count;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_RTN_HISTO_LONG_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_rtn_hist_long;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_RTN_HISTO_MED_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_rtn_hist_med;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_RTN_HISTO_SHORT_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_rtn_hist_short;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_REF_HISTO_LONG_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_ref_hist_long;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_REF_HISTO_MED_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_ref_hist_med;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_REF_HISTO_SHORT_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_ref_hist_short;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_DETECT_MIN_VALID_RANGE_MM:
      *ptuning_parm_value = (int32_t)(
                              pdev->xtalk_cfg.algo__crosstalk_detect_min_valid_range_mm);
      break;
    case VL53L4CX_TUNINGPARM_XTALK_DETECT_MAX_VALID_RANGE_MM:
      *ptuning_parm_value = (int32_t)(
                              pdev->xtalk_cfg.algo__crosstalk_detect_max_valid_range_mm);
      break;
    case VL53L4CX_TUNINGPARM_XTALK_DETECT_MAX_SIGMA_MM:
      *ptuning_parm_value =
        (int32_t)pdev->xtalk_cfg.algo__crosstalk_detect_max_sigma_mm;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_DETECT_MIN_MAX_TOLERANCE:
      *ptuning_parm_value =
        (int32_t)pHP->algo__crosstalk_detect_min_max_tolerance;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_DETECT_MAX_VALID_RATE_KCPS:
      *ptuning_parm_value = (int32_t)(
                              pdev->xtalk_cfg.algo__crosstalk_detect_max_valid_rate_kcps);
      break;
    case VL53L4CX_TUNINGPARM_XTALK_DETECT_EVENT_SIGMA:
      *ptuning_parm_value =
        (int32_t)pHP->algo__crosstalk_detect_event_sigma;
      break;
    case VL53L4CX_TUNINGPARM_HIST_XTALK_MARGIN_KCPS:
      *ptuning_parm_value =
        (int32_t)pdev->xtalk_cfg.histogram_mode_crosstalk_margin_kcps;
      break;
    case VL53L4CX_TUNINGPARM_CONSISTENCY_LITE_PHASE_TOLERANCE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_consistency_lite_phase_tolerance;
      break;
    case VL53L4CX_TUNINGPARM_PHASECAL_TARGET:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_phasecal_target;
      break;
    case VL53L4CX_TUNINGPARM_LITE_CAL_REPEAT_RATE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_cal_repeat_rate;
      break;
    case VL53L4CX_TUNINGPARM_LITE_RANGING_GAIN_FACTOR:
      *ptuning_parm_value =
        (int32_t)pdev->gain_cal.standard_ranging_gain_factor;
      break;
    case VL53L4CX_TUNINGPARM_LITE_MIN_CLIP_MM:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_min_clip;
      break;
    case VL53L4CX_TUNINGPARM_LITE_LONG_SIGMA_THRESH_MM:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_long_sigma_thresh_mm;
      break;
    case VL53L4CX_TUNINGPARM_LITE_MED_SIGMA_THRESH_MM:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_med_sigma_thresh_mm;
      break;
    case VL53L4CX_TUNINGPARM_LITE_SHORT_SIGMA_THRESH_MM:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_short_sigma_thresh_mm;
      break;
    case VL53L4CX_TUNINGPARM_LITE_LONG_MIN_COUNT_RATE_RTN_MCPS:
      *ptuning_parm_value = (int32_t)(
                              pdev->tuning_parms.tp_lite_long_min_count_rate_rtn_mcps);
      break;
    case VL53L4CX_TUNINGPARM_LITE_MED_MIN_COUNT_RATE_RTN_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_med_min_count_rate_rtn_mcps;
      break;
    case VL53L4CX_TUNINGPARM_LITE_SHORT_MIN_COUNT_RATE_RTN_MCPS:
      *ptuning_parm_value = (int32_t)(
                              pdev->tuning_parms.tp_lite_short_min_count_rate_rtn_mcps);
      break;
    case VL53L4CX_TUNINGPARM_LITE_SIGMA_EST_PULSE_WIDTH:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_sigma_est_pulse_width_ns;
      break;
    case VL53L4CX_TUNINGPARM_LITE_SIGMA_EST_AMB_WIDTH_NS:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_sigma_est_amb_width_ns;
      break;
    case VL53L4CX_TUNINGPARM_LITE_SIGMA_REF_MM:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_sigma_ref_mm;
      break;
    case VL53L4CX_TUNINGPARM_LITE_RIT_MULT:
      *ptuning_parm_value =
        (int32_t)pdev->xtalk_cfg.crosstalk_range_ignore_threshold_mult;
      break;
    case VL53L4CX_TUNINGPARM_LITE_SEED_CONFIG:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_seed_cfg;
      break;
    case VL53L4CX_TUNINGPARM_LITE_QUANTIFIER:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_quantifier;
      break;
    case VL53L4CX_TUNINGPARM_LITE_FIRST_ORDER_SELECT:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_lite_first_order_select;
      break;
    case VL53L4CX_TUNINGPARM_LITE_XTALK_MARGIN_KCPS:
      *ptuning_parm_value =
        (int32_t)pdev->xtalk_cfg.lite_mode_crosstalk_margin_kcps;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_RTN_LITE_LONG_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_rtn_lite_long;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_RTN_LITE_MED_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_rtn_lite_med;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_RTN_LITE_SHORT_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_rtn_lite_short;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_REF_LITE_LONG_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_ref_lite_long;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_REF_LITE_MED_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_ref_lite_med;
      break;
    case VL53L4CX_TUNINGPARM_INITIAL_PHASE_REF_LITE_SHORT_RANGE:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_init_phase_ref_lite_short;
      break;
    case VL53L4CX_TUNINGPARM_TIMED_SEED_CONFIG:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_timed_seed_cfg;
      break;
    case VL53L4CX_TUNINGPARM_DMAX_CFG_SIGNAL_THRESH_SIGMA:
      *ptuning_parm_value =
        (int32_t)pdev->dmax_cfg.signal_thresh_sigma;
      break;
    case VL53L4CX_TUNINGPARM_DMAX_CFG_REFLECTANCE_ARRAY_0:
      *ptuning_parm_value =
        (int32_t)pdev->dmax_cfg.target_reflectance_for_dmax_calc[0];
      break;
    case VL53L4CX_TUNINGPARM_DMAX_CFG_REFLECTANCE_ARRAY_1:
      *ptuning_parm_value =
        (int32_t)pdev->dmax_cfg.target_reflectance_for_dmax_calc[1];
      break;
    case VL53L4CX_TUNINGPARM_DMAX_CFG_REFLECTANCE_ARRAY_2:
      *ptuning_parm_value =
        (int32_t)pdev->dmax_cfg.target_reflectance_for_dmax_calc[2];
      break;
    case VL53L4CX_TUNINGPARM_DMAX_CFG_REFLECTANCE_ARRAY_3:
      *ptuning_parm_value =
        (int32_t)pdev->dmax_cfg.target_reflectance_for_dmax_calc[3];
      break;
    case VL53L4CX_TUNINGPARM_DMAX_CFG_REFLECTANCE_ARRAY_4:
      *ptuning_parm_value =
        (int32_t)pdev->dmax_cfg.target_reflectance_for_dmax_calc[4];
      break;
    case VL53L4CX_TUNINGPARM_VHV_LOOPBOUND:
      *ptuning_parm_value =
        (int32_t)pdev->stat_nvm.vhv_config__timeout_macrop_loop_bound;
      break;
    case VL53L4CX_TUNINGPARM_REFSPADCHAR_DEVICE_TEST_MODE:
      *ptuning_parm_value =
        (int32_t)pdev->refspadchar.device_test_mode;
      break;
    case VL53L4CX_TUNINGPARM_REFSPADCHAR_VCSEL_PERIOD:
      *ptuning_parm_value =
        (int32_t)pdev->refspadchar.VL53L4CX_p_005;
      break;
    case VL53L4CX_TUNINGPARM_REFSPADCHAR_PHASECAL_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->refspadchar.timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_REFSPADCHAR_TARGET_COUNT_RATE_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->refspadchar.target_count_rate_mcps;
      break;
    case VL53L4CX_TUNINGPARM_REFSPADCHAR_MIN_COUNTRATE_LIMIT_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->refspadchar.min_count_rate_limit_mcps;
      break;
    case VL53L4CX_TUNINGPARM_REFSPADCHAR_MAX_COUNTRATE_LIMIT_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->refspadchar.max_count_rate_limit_mcps;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_EXTRACT_NUM_OF_SAMPLES:
      *ptuning_parm_value =
        (int32_t)pXC->num_of_samples;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_EXTRACT_MIN_FILTER_THRESH_MM:
      *ptuning_parm_value =
        (int32_t)pXC->algo__crosstalk_extract_min_valid_range_mm;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_EXTRACT_MAX_FILTER_THRESH_MM:
      *ptuning_parm_value =
        (int32_t)pXC->algo__crosstalk_extract_max_valid_range_mm;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_EXTRACT_DSS_RATE_MCPS:
      *ptuning_parm_value =
        (int32_t)pXC->dss_config__target_total_rate_mcps;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_EXTRACT_PHASECAL_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pXC->phasecal_config_timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_EXTRACT_MAX_VALID_RATE_KCPS:
      *ptuning_parm_value =
        (int32_t)pXC->algo__crosstalk_extract_max_valid_rate_kcps;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_EXTRACT_SIGMA_THRESHOLD_MM:
      *ptuning_parm_value =
        (int32_t)pXC->algo__crosstalk_extract_max_sigma_mm;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_EXTRACT_DSS_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pXC->mm_config_timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_XTALK_EXTRACT_BIN_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pXC->range_config_timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_OFFSET_CAL_DSS_RATE_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->offsetcal_cfg.dss_config__target_total_rate_mcps;
      break;
    case VL53L4CX_TUNINGPARM_OFFSET_CAL_PHASECAL_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->offsetcal_cfg.phasecal_config_timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_OFFSET_CAL_MM_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->offsetcal_cfg.mm_config_timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_OFFSET_CAL_RANGE_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->offsetcal_cfg.range_config_timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_OFFSET_CAL_PRE_SAMPLES:
      *ptuning_parm_value =
        (int32_t)pdev->offsetcal_cfg.pre_num_of_samples;
      break;
    case VL53L4CX_TUNINGPARM_OFFSET_CAL_MM1_SAMPLES:
      *ptuning_parm_value =
        (int32_t)pdev->offsetcal_cfg.mm1_num_of_samples;
      break;
    case VL53L4CX_TUNINGPARM_OFFSET_CAL_MM2_SAMPLES:
      *ptuning_parm_value =
        (int32_t)pdev->offsetcal_cfg.mm2_num_of_samples;
      break;
    case VL53L4CX_TUNINGPARM_ZONE_CAL_DSS_RATE_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->zonecal_cfg.dss_config__target_total_rate_mcps;
      break;
    case VL53L4CX_TUNINGPARM_ZONE_CAL_PHASECAL_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->zonecal_cfg.phasecal_config_timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_ZONE_CAL_DSS_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->zonecal_cfg.mm_config_timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_ZONE_CAL_PHASECAL_NUM_SAMPLES:
      *ptuning_parm_value =
        (int32_t)pdev->zonecal_cfg.phasecal_num_of_samples;
      break;
    case VL53L4CX_TUNINGPARM_ZONE_CAL_RANGE_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->zonecal_cfg.range_config_timeout_us;
      break;
    case VL53L4CX_TUNINGPARM_ZONE_CAL_ZONE_NUM_SAMPLES:
      *ptuning_parm_value =
        (int32_t)pdev->zonecal_cfg.zone_num_of_samples;
      break;
    case VL53L4CX_TUNINGPARM_SPADMAP_VCSEL_PERIOD:
      *ptuning_parm_value =
        (int32_t)pdev->ssc_cfg.VL53L4CX_p_005;
      break;
    case VL53L4CX_TUNINGPARM_SPADMAP_VCSEL_START:
      *ptuning_parm_value =
        (int32_t)pdev->ssc_cfg.vcsel_start;
      break;
    case VL53L4CX_TUNINGPARM_SPADMAP_RATE_LIMIT_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->ssc_cfg.rate_limit_mcps;
      break;
    case VL53L4CX_TUNINGPARM_LITE_DSS_CONFIG_TARGET_TOTAL_RATE_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_dss_target_lite_mcps;
      break;
    case VL53L4CX_TUNINGPARM_RANGING_DSS_CONFIG_TARGET_TOTAL_RATE_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_dss_target_histo_mcps;
      break;
    case VL53L4CX_TUNINGPARM_MZ_DSS_CONFIG_TARGET_TOTAL_RATE_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_dss_target_histo_mz_mcps;
      break;
    case VL53L4CX_TUNINGPARM_TIMED_DSS_CONFIG_TARGET_TOTAL_RATE_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_dss_target_timed_mcps;
      break;
    case VL53L4CX_TUNINGPARM_LITE_PHASECAL_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_phasecal_timeout_lite_us;
      break;
    case VL53L4CX_TUNINGPARM_RANGING_LONG_PHASECAL_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_phasecal_timeout_hist_long_us;
      break;
    case VL53L4CX_TUNINGPARM_RANGING_MED_PHASECAL_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_phasecal_timeout_hist_med_us;
      break;
    case VL53L4CX_TUNINGPARM_RANGING_SHORT_PHASECAL_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_phasecal_timeout_hist_short_us;
      break;
    case VL53L4CX_TUNINGPARM_MZ_LONG_PHASECAL_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_phasecal_timeout_mz_long_us;
      break;
    case VL53L4CX_TUNINGPARM_MZ_MED_PHASECAL_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_phasecal_timeout_mz_med_us;
      break;
    case VL53L4CX_TUNINGPARM_MZ_SHORT_PHASECAL_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_phasecal_timeout_mz_short_us;
      break;
    case VL53L4CX_TUNINGPARM_TIMED_PHASECAL_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_phasecal_timeout_timed_us;
      break;
    case VL53L4CX_TUNINGPARM_LITE_MM_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_mm_timeout_lite_us;
      break;
    case VL53L4CX_TUNINGPARM_RANGING_MM_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_mm_timeout_histo_us;
      break;
    case VL53L4CX_TUNINGPARM_MZ_MM_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_mm_timeout_mz_us;
      break;
    case VL53L4CX_TUNINGPARM_TIMED_MM_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_mm_timeout_timed_us;
      break;
    case VL53L4CX_TUNINGPARM_LITE_RANGE_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_range_timeout_lite_us;
      break;
    case VL53L4CX_TUNINGPARM_RANGING_RANGE_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_range_timeout_histo_us;
      break;
    case VL53L4CX_TUNINGPARM_MZ_RANGE_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_range_timeout_mz_us;
      break;
    case VL53L4CX_TUNINGPARM_TIMED_RANGE_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_range_timeout_timed_us;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_SMUDGE_MARGIN:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.smudge_margin;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_NOISE_MARGIN:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.noise_margin;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_XTALK_OFFSET_LIMIT:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.user_xtalk_offset_limit;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_XTALK_OFFSET_LIMIT_HI:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.user_xtalk_offset_limit_hi;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_SAMPLE_LIMIT:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.sample_limit;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_SINGLE_XTALK_DELTA:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.single_xtalk_delta;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_AVERAGED_XTALK_DELTA:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.averaged_xtalk_delta;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_CLIP_LIMIT:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.smudge_corr_clip_limit;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_SCALER_CALC_METHOD:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.scaler_calc_method;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_XGRADIENT_SCALER:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.x_gradient_scaler;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_YGRADIENT_SCALER:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.y_gradient_scaler;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_USER_SCALER_SET:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.user_scaler_set;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_SMUDGE_COR_SINGLE_APPLY:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.smudge_corr_single_apply;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_XTALK_AMB_THRESHOLD:
      *ptuning_parm_value = (int32_t)(
                              pdev->smudge_correct_config.smudge_corr_ambient_threshold);
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_NODETECT_AMB_THRESHOLD_KCPS:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.nodetect_ambient_threshold;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_NODETECT_SAMPLE_LIMIT:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.nodetect_sample_limit;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_NODETECT_XTALK_OFFSET_KCPS:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.nodetect_xtalk_offset;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_NODETECT_MIN_RANGE_MM:
      *ptuning_parm_value =
        (int32_t)pdev->smudge_correct_config.nodetect_min_range_mm;
      break;
    case VL53L4CX_TUNINGPARM_LOWPOWERAUTO_VHV_LOOP_BOUND:
      *ptuning_parm_value =
        (int32_t)pdev->low_power_auto_data.vhv_loop_bound;
      break;
    case VL53L4CX_TUNINGPARM_LOWPOWERAUTO_MM_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_mm_timeout_lpa_us;
      break;
    case VL53L4CX_TUNINGPARM_LOWPOWERAUTO_RANGE_CONFIG_TIMEOUT_US:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_range_timeout_lpa_us;
      break;
    case VL53L4CX_TUNINGPARM_VERY_SHORT_DSS_RATE_MCPS:
      *ptuning_parm_value =
        (int32_t)pdev->tuning_parms.tp_dss_target_very_short_mcps;
      break;
    case VL53L4CX_TUNINGPARM_PHASECAL_PATCH_POWER:
      *ptuning_parm_value =
        (int32_t) pdev->tuning_parms.tp_phasecal_patch_power;
      break;
    case VL53L4CX_TUNINGPARM_HIST_MERGE:
      *ptuning_parm_value =
        (int32_t) pdev->tuning_parms.tp_hist_merge;
      break;
    case VL53L4CX_TUNINGPARM_RESET_MERGE_THRESHOLD:
      *ptuning_parm_value =
        (int32_t) pdev->tuning_parms.tp_reset_merge_threshold;
      break;
    case VL53L4CX_TUNINGPARM_HIST_MERGE_MAX_SIZE:
      *ptuning_parm_value =
        (int32_t) pdev->tuning_parms.tp_hist_merge_max_size;
      break;
    case VL53L4CX_TUNINGPARM_DYNXTALK_MAX_SMUDGE_FACTOR:
      *ptuning_parm_value =
        pdev->smudge_correct_config.max_smudge_factor;
      break;

    case VL53L4CX_TUNINGPARM_UWR_ENABLE:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_enable;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_1_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_1_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_1_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_1_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_2_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_2_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_2_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_2_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_3_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_3_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_3_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_3_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_4_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_4_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_4_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_4_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_5_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_5_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_ZONE_5_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_z_5_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_1_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_1_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_1_RANGEB:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_1_rangeb;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_2_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_2_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_2_RANGEB:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_2_rangeb;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_3_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_3_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_3_RANGEB:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_3_rangeb;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_4_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_4_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_4_RANGEB:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_4_rangeb;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_5_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_5_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_MEDIUM_CORRECTION_ZONE_5_RANGEB:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_med_corr_z_5_rangeb;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_1_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_1_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_1_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_1_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_2_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_2_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_2_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_2_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_3_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_3_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_3_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_3_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_4_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_4_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_4_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_4_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_5_MIN:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_5_min;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_ZONE_5_MAX:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_z_5_max;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_1_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_corr_z_1_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_1_RANGEB:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_corr_z_1_rangeb;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_2_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_corr_z_2_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_2_RANGEB:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_corr_z_2_rangeb;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_3_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_corr_z_3_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_3_RANGEB:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_corr_z_3_rangeb;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_4_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_corr_z_4_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_4_RANGEB:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_corr_z_4_rangeb;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_5_RANGEA:
      *ptuning_parm_value =
        pdev->tuning_parms.tp_uwr_lng_corr_z_5_rangea;
      break;
    case VL53L4CX_TUNINGPARM_UWR_LONG_CORRECTION_ZONE_5_RANGEB:
      *ptuning_parm_value =
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_tuning_parm(
  VL53L4CX_DEV            Dev,
  VL53L4CX_TuningParms    tuning_parm_key,
  ...
  switch (tuning_parm_key) {
    ...
    case VL53L4CX_TUNINGPARM_KEY_TABLE_VERSION:
      ...
      if ((uint16_t)tuning_parm_value
        ...
        status = VL53L4CX_ERROR_TUNING_PARM_KEY_MISMATCH;
      }
    ...
    case VL53L4CX_TUNINGPARM_HIST_ALGO_SELECT:
      pHP->hist_algo_select =
        (VL53L4CX_HistAlgoSelect)tuning_parm_value;
      break;
    case VL53L4CX_TUNINGPARM_HIST_TARGET_ORDER:
      pHP->hist_target_order =
        (VL53L4CX_HistTargetOrder)tuning_parm_value;
      break;
    case VL53L4CX_TUNINGPARM_HIST_FILTER_WOI_0:
      pHP->filter_woi0 =
    ...
    case VL53L4CX_TUNINGPARM_HIST_AMB_EST_METHOD:
      pHP->hist_amb_est_method =
        (VL53L4CX_HistAmbEstMethod)tuning_parm_value;
      break;
    case VL53L4CX_TUNINGPARM_HIST_AMB_THRESH_SIGMA_0:
      pHP->ambient_thresh_sigma0 =
        (uint8_t)tuning_parm_value;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_enable(
  VL53L4CX_DEV                          Dev
)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_disable(
  VL53L4CX_DEV                          Dev
)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_apply_disable(
  VL53L4CX_DEV                          Dev
)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_single_apply_enable(
  VL53L4CX_DEV                          Dev
)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_single_apply_disable(
  VL53L4CX_DEV                          Dev
)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_dynamic_xtalk_correction_apply_enable(
  VL53L4CX_DEV                          Dev
)
{
...
VL53L4CX_Error VL53L4CX::VL53L4CX_get_current_xtalk_settings(
  VL53L4CX_DEV                          Dev,
...
VL53L4CX_Error VL53L4CX::VL53L4CX_set_current_xtalk_settings(
  VL53L4CX_DEV                          Dev,
  VL53L4CX_xtalk_calibration_results_t *pxtalk
)
{
