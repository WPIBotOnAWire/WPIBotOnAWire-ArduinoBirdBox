...
VL53L4CX_Error VL53L4CX::VL53L4CX_xtalk_calibration_process_data(
  ...
  VL53L4CX_xtalk_calibration_results_t  *pxtalk_cal)
{
  ...
  VL53L4CX_xtalk_algo_data_t xtalk_debug;
  VL53L4CX_xtalk_algo_data_t *pdebug      = &xtalk_debug;
  VL53L4CX_xtalk_range_data_t *pxtalk_data = NULL;

  VL53L4CX_histogram_bin_data_t avg_bins;
  VL53L4CX_histogram_bin_data_t *pavg_bins   = &avg_bins;
  ...
  if (status == VL53L4CX_ERROR_NONE)
    VL53L4CX_init_histogram_bin_data_struct(
      0, 0, &(pdebug->VL53L4CX_p_056));

  if (status == VL53L4CX_ERROR_NONE)
    VL53L4CX_init_histogram_bin_data_struct(
      0, 0, &(pdebug->VL53L4CX_p_057));



  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_f_039(
               pxtalk_results,
               pdebug,
  ...
  pxtalk_data = &(pxtalk_results->VL53L4CX_p_003[4]);
...
VL53L4CX_Error VL53L4CX::VL53L4CX_f_041(
...
VL53L4CX_Error VL53L4CX::VL53L4CX_f_039(
  VL53L4CX_xtalk_range_results_t   *pxtalk_results,
  VL53L4CX_xtalk_algo_data_t       *pdebug,
  int16_t                        *xgradient,
  int16_t                        *ygradient
)
{
  ...
  VL53L4CX_xtalk_range_data_t  *presults_int = NULL;

  int          i                   = 0;

  uint32_t xtalk_per_spad[4];
  int32_t  VL53L4CX_p_059         = 0;
  int32_t  VL53L4CX_p_060         = 0;
  ...
  *ygradient = 0;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (status == VL53L4CX_ERROR_NONE) {
      ...
      presults_int = &(pxtalk_results->VL53L4CX_p_003[i]);
  ...
  if ((status == VL53L4CX_ERROR_NONE) && (result_invalid == 0)) {



    if (status == VL53L4CX_ERROR_NONE) {
      VL53L4CX_p_059 = ((int32_t)xtalk_per_spad[1]
                        - (int32_t)xtalk_per_spad[0]) / (8);
      VL53L4CX_p_060 = ((int32_t)xtalk_per_spad[3]
    ...
    if (status == VL53L4CX_ERROR_NONE) {
      if (VL53L4CX_p_059 < -32767) {
        VL53L4CX_p_059 = -32767;
      } else {
        if (VL53L4CX_p_059 > 32767) {
          VL53L4CX_p_059 = 32767;
        }
      ...
      if (VL53L4CX_p_060 < -32767) {
        VL53L4CX_p_060 = -32767;
      } else {
        if (VL53L4CX_p_060 > 32767) {
          VL53L4CX_p_060 = 32767;
        }
      ...
      pdebug->VL53L4CX_p_059 = (int16_t)VL53L4CX_p_059;
      pdebug->VL53L4CX_p_060 = (int16_t)VL53L4CX_p_060;
    }
    ...
    pdebug->VL53L4CX_p_059 = 0;
    pdebug->VL53L4CX_p_060 = 0;
  }



  if (status == VL53L4CX_ERROR_NONE) {
    *xgradient = (int16_t)VL53L4CX_p_059;
    *ygradient = (int16_t)VL53L4CX_p_060;
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_f_040(
  VL53L4CX_xtalk_range_data_t *pxtalk_data,
  VL53L4CX_xtalk_algo_data_t  *pdebug,
...
)
{
  ...
  *xtalk_mean_offset_kcps          = 0;
  ...
  if ((status == VL53L4CX_ERROR_NONE) && (result_invalid == 0)) {
    ...
    pdebug->VL53L4CX_p_058 = xtalk_per_spad;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_f_045(
...
)
{
  ...
  int32_t tmpi32;
  uint8_t i = 0;
  uint64_t bin_data[VL53L4CX_XTALK_HISTO_BINS];

  pxtalk_histo->VL53L4CX_p_020             =
    phist_data->VL53L4CX_p_020;
  pxtalk_histo->cal_config__vcsel_start =
    phist_data->cal_config__vcsel_start;
  pxtalk_histo->VL53L4CX_p_015     =
    phist_data->VL53L4CX_p_015;
  pxtalk_histo->VL53L4CX_p_019               =
    phist_data->VL53L4CX_p_019;
  ...
  memcpy(&(pdebug->VL53L4CX_p_057),
         phist_data, sizeof(VL53L4CX_histogram_bin_data_t));
  ...
  if (pxtalk_data->signal_total_events_avg == 0) {
    for (i = 0; i < pxtalk_histo->VL53L4CX_p_021; i++) {
      bin_data[i] = 0;
    }
    goto FAIL;
  }
...
FAIL:
  ...
  for (i = 0; i < pxtalk_histo->VL53L4CX_p_021; i++) {
    pdebug->VL53L4CX_p_062[i] = pxtalk_histo->bin_data[i];
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_f_033(
  VL53L4CX_histogram_bin_data_t    *phist_data,
...
)
{
  ...
  for (i = 0; i < pxtalk_data->VL53L4CX_p_021; i++) {
...
VL53L4CX_Error VL53L4CX::VL53L4CX_f_047(
  VL53L4CX_histogram_bin_data_t *phist_data,
  ...
  uint8_t         xtalk_bin_offset)
{
  ...
  if (status == VL53L4CX_ERROR_NONE)
    for (i = xtalk_bin_offset;
         i < pxtalk_data->VL53L4CX_p_021; i++) {
...
VL53L4CX_Error VL53L4CX::VL53L4CX_f_044(
  VL53L4CX_histogram_bin_data_t   *pxtalk_data,
  ...
  uint8_t           VL53L4CX_p_024)
{
  ...
  int32_t realign_bin_data[VL53L4CX_HISTOGRAM_BUFFER_SIZE];

  for (i = 0 ; i < VL53L4CX_HISTOGRAM_BUFFER_SIZE ; i++) {
    realign_bin_data[i] = 0;
  }

  first_bin_int = VL53L4CX_p_019;
  last_bin_int  = VL53L4CX_p_024;





  VL53L4CX_hist_remove_ambient_bins(pxtalk_data);



  first_bin_int = (first_bin_int) %
                  pxtalk_data->VL53L4CX_p_021;

  last_bin_int = (last_bin_int) %
                 pxtalk_data->VL53L4CX_p_021;

  first_bin_inc = (first_bin_int + 1) % pxtalk_data->VL53L4CX_p_021;
  ...
  if (first_bin_inc > last_bin_int) {
    ...
    realign_bin = pxtalk_data->VL53L4CX_p_021 - first_bin_inc;
  ...
  if (realign_bin > 0) {
    ...
    for (i = 0; i < pxtalk_data->VL53L4CX_p_021; i++) {
      realign_index = (pxtalk_data->VL53L4CX_p_021 -
                       realign_bin + i)
                      % pxtalk_data->VL53L4CX_p_021;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_f_043(
  uint8_t                      sigma_mult,
  ...
  uint32_t                    *ambient_noise)
{
  ...
  if (VL53L4CX_p_028 <= 0) {
    ...
    ambient_events_per_bin_int = (uint32_t)VL53L4CX_p_028;
  }

  *ambient_noise =  VL53L4CX_isqrt(ambient_events_per_bin_int);
