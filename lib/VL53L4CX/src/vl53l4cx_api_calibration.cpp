...
VL53L4CX_Error VL53L4CX::VL53L4CX_run_ref_spad_char(
  VL53L4CX_DEV        Dev,
  VL53L4CX_Error     *pcal_status)
{
  ...
  VL53L4CX_refspadchar_config_t *prefspadchar  = &(pdev->refspadchar);
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_run_device_test(
               Dev,
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status =
      VL53L4CX_ReadMulti(
        Dev,
        VL53L4CX_REF_SPAD_CHAR_RESULT__NUM_ACTUAL_REF_SPADS,
        comms_buffer,
        2);
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    switch (pdev->sys_results.result__range_status) {
      ...
      case VL53L4CX_DEVICEERROR_REFSPADCHARNOTENOUGHDPADS:
        status = VL53L4CX_WARNING_REF_SPAD_CHAR_NOT_ENOUGH_SPADS;
        break;

      case VL53L4CX_DEVICEERROR_REFSPADCHARMORETHANTARGET:
        status = VL53L4CX_WARNING_REF_SPAD_CHAR_RATE_TOO_HIGH;
        break;

      case VL53L4CX_DEVICEERROR_REFSPADCHARLESSTHANTARGET:
        status = VL53L4CX_WARNING_REF_SPAD_CHAR_RATE_TOO_LOW;
        break;
    }
  }
  ...
  *pcal_status = status;
  ...
  IGNORE_STATUS(
    ...
    VL53L4CX_WARNING_REF_SPAD_CHAR_NOT_ENOUGH_SPADS,
    status);

  IGNORE_STATUS(
    IGNORE_REF_SPAD_CHAR_RATE_TOO_HIGH,
    VL53L4CX_WARNING_REF_SPAD_CHAR_RATE_TOO_HIGH,
    status);

  IGNORE_STATUS(
    IGNORE_REF_SPAD_CHAR_RATE_TOO_LOW,
    VL53L4CX_WARNING_REF_SPAD_CHAR_RATE_TOO_LOW,
    status);
...
VL53L4CX_Error VL53L4CX::VL53L4CX_get_and_avg_xtalk_samples(
  ...
  VL53L4CX_xtalk_range_results_t *pXR,
  ...
  VL53L4CX_histogram_bin_data_t  *pavg_histo)
{
  ...
  VL53L4CX_range_results_t      *prs =
    (VL53L4CX_range_results_t *) pdev->wArea1;

  VL53L4CX_range_data_t         *prange_data;
  VL53L4CX_xtalk_range_data_t   *pxtalk_range_data;
  ...
  status = VL53L4CX_dynamic_xtalk_correction_disable(Dev);
  ...
  for (i = 0; i <= (final_zone * num_of_samples); i++) {
    ...
    if (status == VL53L4CX_ERROR_NONE) {
      status = VL53L4CX_wait_for_range_completion(Dev);
    }
    ...
    if (status == VL53L4CX_ERROR_NONE &&
      ...
      prange_data       = &(prs->VL53L4CX_p_003[0]);


      if (prs->active_results > 1) {
        for (j = 1;
             j < prs->active_results; j++) {
          if (prs->VL53L4CX_p_003[j].median_range_mm
              <
              prange_data->median_range_mm)
            prange_data =
              &(prs->VL53L4CX_p_003[j]);

        }
      }

      pxtalk_range_data = &(pXR->VL53L4CX_p_003[zone_id]);
      ...
      if (valid_result == 1) {
        ...
        pxtalk_range_data->signal_total_events_sum +=
          prange_data->VL53L4CX_p_010;

        pxtalk_range_data->sigma_mm_sum +=
          (uint32_t)prange_data->VL53L4CX_p_002;



        pxtalk_range_data->median_phase_sum +=
          (uint32_t)prange_data->VL53L4CX_p_011;
      ...
      if ((valid_result == 1) && (zone_id >= 4)) {
        status = VL53L4CX_sum_histogram_data(
                   &(pdev->hist_data),
                   psum_histo);



        if (prange_data->VL53L4CX_p_012 <
            pXR->central_histogram__window_start)
          pXR->central_histogram__window_start =
            prange_data->VL53L4CX_p_012;


        if (prange_data->VL53L4CX_p_013 >
            pXR->central_histogram__window_end)
          pXR->central_histogram__window_end =
            prange_data->VL53L4CX_p_013;
    ...
    if (status == VL53L4CX_ERROR_NONE)
      status =
        VL53L4CX_clear_interrupt_and_enable_next_range(
          Dev,
          measurement_mode);
  }
  ...
  for (i = 0; i < (pdev->zone_cfg.active_zones + 1); i++) {
    ...
    pxtalk_range_data = &(pXR->VL53L4CX_p_003[i + xtalk_result_id]);
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    ...
    pxtalk_range_data = &(pXR->VL53L4CX_p_003[xtalk_histo_id]);

    status = VL53L4CX_avg_histogram_data(
               pxtalk_range_data->no_of_samples,
               psum_histo,
               pavg_histo);
  }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    if (smudge_corr_en == 1) {
      status = VL53L4CX_dynamic_xtalk_correction_enable(Dev);
    }
  }
  ...
  uint16_t               *pzero_distance_phase)
{
  ...
  for (m = 0; m < phasecal_num_of_samples; m++) {
    ...
    for (i = 0; i <= 1; i++) {
      ...
      if (status == VL53L4CX_ERROR_NONE)
        status =
          VL53L4CX_wait_for_range_completion(Dev);
    ...
    if (status == VL53L4CX_ERROR_NONE) {
      status = VL53L4CX_WaitUs(Dev, 1000);
    }
    ...
    if (status == VL53L4CX_ERROR_NONE) {
      ...
      period = 2048 *
               (uint32_t)VL53L4CX_decode_vcsel_period(
                 pdev->hist_data.VL53L4CX_p_005);

      VL53L4CX_p_014  = period;
      VL53L4CX_p_014 += (uint32_t)(
      ...
      if (period != 0) {
        VL53L4CX_p_014  = VL53L4CX_p_014 % period;
      } else {
        status =
      ...
      zero_distance_phase += (uint32_t)VL53L4CX_p_014;
    }
  }
  VL53L4CX_unload_patch(Dev);
  ...
  if (status == VL53L4CX_ERROR_NONE && samples > 0) {
  ...
  VL53L4CX_DeviceTestMode  device_test_mode)
{
  ...
  if (status == VL53L4CX_ERROR_NONE)
    status = VL53L4CX_start_test(
               Dev,
               device_test_mode);


  if (status == VL53L4CX_ERROR_NONE) {
    status = VL53L4CX_wait_for_test_completion(Dev);
  }
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    if (status == VL53L4CX_ERROR_NONE) {
      status = VL53L4CX_clear_interrupt(Dev);
    }
  }
...
void VL53L4CX::VL53L4CX_hist_xtalk_extract_data_init(
  VL53L4CX_hist_xtalk_extract_data_t *pxtalk_data)
{
  ...
  pxtalk_data->VL53L4CX_p_012             = 0U;
  pxtalk_data->VL53L4CX_p_013               = 0U;
...
VL53L4CX_Error VL53L4CX::VL53L4CX_hist_xtalk_extract_update(
  ...
  VL53L4CX_hist_xtalk_extract_data_t   *pxtalk_data)
{
  ...
  status =
    VL53L4CX_hist_xtalk_extract_calc_window(
      target_distance_mm,
  ...
  if (status == VL53L4CX_ERROR_NONE) {
    status =
      VL53L4CX_hist_xtalk_extract_calc_event_sums(
        phist_bins,
        pxtalk_data);
  }
...
VL53L4CX_Error VL53L4CX::VL53L4CX_hist_xtalk_extract_fini(
  VL53L4CX_histogram_bin_data_t        *phist_bins,
  VL53L4CX_hist_xtalk_extract_data_t   *pxtalk_data,
  ...
  VL53L4CX_xtalk_histogram_shape_t     *pxtalk_shape)
{
  ...
  VL53L4CX_Error  status = VL53L4CX_ERROR_NONE;
  VL53L4CX_xtalk_calibration_results_t *pX = pxtalk_cal;
  ...
  if (pxtalk_data->sample_count > 0) {
    ...
    status =
      VL53L4CX_hist_xtalk_extract_calc_rate_per_spad(
        pxtalk_data);
    ...
    if (status == VL53L4CX_ERROR_NONE) {
      ...
      status =
        VL53L4CX_hist_xtalk_extract_calc_shape(
          pxtalk_data,
          pxtalk_shape);
      ...
      pxtalk_shape->VL53L4CX_p_015 =
        phist_bins->VL53L4CX_p_015;
    }
...
VL53L4CX_Error   VL53L4CX::VL53L4CX_run_hist_xtalk_extraction(
  VL53L4CX_DEV                          Dev,
...
#define OVERSIZE 4
  VL53L4CX_Error status = VL53L4CX_ERROR_NONE;
  VL53L4CX_LLDriverData_t *pdev = VL53L4CXDevStructGetLLDriverHandle(Dev);
  VL53L4CX_xtalkextract_config_t *pX = &(pdev->xtalk_extract_cfg);
  VL53L4CX_xtalk_config_t *pC = &(pdev->xtalk_cfg);
  VL53L4CX_xtalk_calibration_results_t *pXC = &(pdev->xtalk_cal);
  ...
  uint32_t XtalkMin;
  uint32_t XtalkMax;
  uint8_t measurement_mode = VL53L4CX_DEVICEMEASUREMENTMODE_BACKTOBACK;
  int8_t MaxId;
  ...
  VL53L4CX_get_tuning_parm(Dev, VL53L4CX_TUNINGPARM_HIST_MERGE_MAX_SIZE,
                           &initMergeSize);
  VL53L4CX_get_tuning_parm(Dev, VL53L4CX_TUNINGPARM_HIST_MERGE,
                           &MergeEnabled);
  ...
  for (k = 0; k < nbloops; k++) {
    ...
    VL53L4CX_hist_xtalk_extract_data_init(
      &(pdev->xtalk_extract));
    VL53L4CX_set_tuning_parm(Dev,
                             VL53L4CX_TUNINGPARM_HIST_MERGE_MAX_SIZE,
                             k * MaxId + 1);
    ...
    for (i = 0; i <= pX->num_of_samples; i++) {
      ...
      if (status == VL53L4CX_ERROR_NONE)
        status = VL53L4CX_get_device_results(Dev,
                                             VL53L4CX_DEVICERESULTSLEVEL_FULL,
                                             prange_results);
      Very1stRange =
        (pdev->ll_state.rd_device_state ==
      ...
      if (wait_for_accumulation) {
        ...
        if ((status == VL53L4CX_ERROR_NONE) &&
            (!Very1stRange)) {
          status =
            VL53L4CX_hist_xtalk_extract_update(
              cal_distance_mm,
              OVERSIZE,
              &(pdev->hist_data),
              &(pdev->xtalk_extract));
    ...
    if (status == VL53L4CX_ERROR_NONE)
      status =
        VL53L4CX_hist_xtalk_extract_fini(
          &(pdev->hist_data),
          &(pdev->xtalk_extract),
          &(pdev->xtalk_cal),
  ...
  VL53L4CX_stop_range(Dev);

  VL53L4CX_set_tuning_parm(Dev, VL53L4CX_TUNINGPARM_HIST_MERGE_MAX_SIZE,
                           initMergeSize);
  VL53L4CX_unload_patch(Dev);

  if (status != VL53L4CX_ERROR_NONE) {
    status = VL53L4CX_ERROR_XTALK_EXTRACTION_SIGMA_LIMIT_FAIL;
